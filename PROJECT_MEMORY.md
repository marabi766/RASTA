...
| `notification-service` | IMPLEMENTED · TESTED · **CI VERIFIED** — `NTF-001` و `NTF-002` روی `main` (`ADR-054`). **`NTF-003` (ترجیحات) روی شاخه است**: ۲۰۵ واحد + ۱۱۰ یکپارچگی محلیِ سبز، **هنوز CI‌ندیده** |
...
روی شاخهٔ `feat/notification-preferences`، **نه روی `main`**. آنچه ساخته شد:
...
**شواهدی که در این نشست خودم اجرا کردم** (محلی، Windows، Postgres و Kafka
واقعی): ۲۰۵ واحد + ۱۱۰ یکپارچگی سبز، و
`verify-migration-reversible.mjs notification` سبز — up → down → up روی Schema
یک‌بارمصرف، با هر چهار Migration این سرویس.
...
_آخرین Audit کامل: 2026-08-27. آخرین به‌روزرسانی: **2026-09-20** — `NTF-003` و
بستن `Q-38` روی شاخهٔ `feat/notification-preferences`، شواهد محلی و هنوز
CI‌ندیده. پیش‌تر: 2026-09-18 — شش Merge، `main` روی `f7252d3`، اجرای سبز
`35364991343`؛ و 2026-08-30 (Baseline سنجش پیشرفت).
