...
| **پورتال کاربر (`apps/web`) — ورود، داشبورد، صفحه‌های دارایی و ثبت کارکرد** |                          ✅                           | ✅ (۴۵۵ واحد + ۶ مرورگری)  | ⚠️ **CI VERIFIED**، نه زنده: Job ‏`Portal in a browser` در برابر Build تولیدی سبز است؛ ورود و صفحه‌های دارایی هرگز در برابر Keycloak و Gateway زنده اجرا نشده‌اند (`ADR-059`)                                                                                              |
...
- Frontend — **بقیهٔ صفحه‌های دامنه‌ای**: از `EXP-002` فقط ورود، داشبورد و دو
  صفحهٔ دارایی ساخته شده؛ `EXP-003` تا `EXP-005` شروع نشده‌اند. `apps/admin`
