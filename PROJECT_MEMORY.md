...
| —     | آزمون `claims pending rows` فرض می‌کند ردیف خودش در ۱۰۰ ردیف نخست است            | پایین                           | با Backlog بزرگ در پایگاه داده مشترک، `claimPending(100)` ردیف تازه را برنمی‌گرداند              | با Churn مصنوعی (۱۰۷۴ ردیف) دیده شد، نه در اجرای عادی؛ همان خانواده D-024 اما روی `LIMIT`                   |
...
**یافته جانبی (ثبت شد، اصلاح نشد):** زیر Churn مصنوعی سنگین (۱۰۷۴ ردیف
در انتظار)، آزمون `claims pending rows` می‌افتد — چون `claimPending(100)` یک
`LIMIT` دارد و ردیف تازه بیرون از پنجره می‌ماند. همان خانواده فرض است، اما روی
`LIMIT` به‌جای `COUNT`؛ در اجرای عادی رخ نمی‌دهد و اصلاحش خارج از دامنه این
Task است.
