...
- Frontend — **بقیهٔ صفحه‌های دامنه‌ای**: از `EXP-002` ورود، داشبورد، سازمان
  و اعضا، دو صفحهٔ دارایی، ثبت کارکرد، دو صفحهٔ نگهداری و دو صفحهٔ راننده
  ساخته شده؛ `/assets/[id]/timeline` (آخرین صفحهٔ P0 این نقشه) روی یک شاخهٔ
  باز، منتظر ادغام. `EXP-003` تا `EXP-005` شروع نشده‌اند. `apps/admin` اصلاً
  وجود ندارد (ADR-058 § ۲)
