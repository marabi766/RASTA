...
| —     | سه پارامتر Query هنوز `z.coerce.boolean()` دارند                                 | متوسط                           | `?availableOnly=false` و `?includeIncoming=false` وارونه پاسخ می‌دهند — همان نقص D-020 روی مرز HTTP | `booleanEnv` آماده است؛ چون پاسخ API را عوض می‌کند، Task مستقل خودش را می‌خواهد                             |
...
**خارج از دامنه، هنوز باز:** سه پارامتر Query هنوز `z.coerce.boolean()`
دارند — `availableOnly` در `asset-service` و `incoming`/`includeIncoming` در
`economic-service`. همان نقص است، اما مرز دیگری (HTTP API، نه محیط) و تغییر
رفتار قابل‌مشاهده API؛ Task مستقل خودش را می‌خواهد. `document-service` این
مسیر را برای `includeDeleted` قبلاً رفته است.
