...
> AUD-004 باز است.
>
> **به‌روزرسانی 2026-09-17 (ADR-055 — Manifest مسیر گزارش برای شمارش Slot و بازبینی Cohort؛ حکم NO-GO):** برای رفع
> سقف ۸۱۹۱ نویسهٔ `cmd.exe` در گام‌های L12 و L13، `account:aggregation-campaign` شکل `--reports-manifest <file>` و
> `review:aggregation-campaign-image-cohort` شکل `<image-cohort-manifest> --reports-manifest <file>` گرفتند؛ شکل‌های
> مسیر صریح حفظ شدند. دستور، کران‌ها (۶۰٬۴۷۵ بایت، دقیقاً ۵۹ خط LF‌دار، هر مسیر ≤ ۱۰۲۴ بایت)، resolve نسبت به پوشهٔ
> Manifest و سیاست تکرار (Windows محافظه‌کارانه، POSIX دقیق) بی‌تغییر از مقایسه‌گر به ماژول مشترک
> `scripts/aggregation-campaign-report-manifest-lib.mjs` منتقل شدند و هر سه فرمان از آن استفاده می‌کنند (بدون رونوشت؛
> خروجی مقایسه‌گر بی‌تغییر). Manifest ‏JSON ‏Cohort جدا، با همان اعتبارسنج و کران ۴ KiB، و همچنان نخستین آرگومان است.
> خطای استفاده/Manifest (آمیختن دو شکل، گزینهٔ تکراری/ناشناخته/Inline، مقدار ناقص، آرگومان اضافه، `--` دیرهنگام، مسیر
> خالی یا بیش از ۱۰۲۴ بایت، بیش از ۱۲۸ مسیر صریح، مسیر تکراری پس از resolve، Manifest خراب/بزرگ/با اندازهٔ نامعلوم/
> رشدیافته/ناخوانا) پیش از خواندن هر گزارش خروج `2` با پیام ثابت می‌دهد. سقف ۱۲۸ (`LOG_RECOVERY_LIMITS.maxLogs`) کران
> موجود بازبینی و مقایسه‌گر است، نه ۵۹، تا گزارش اضافهٔ شصتم همچنان به شمارش برسد (خروج `1`). شمارش و بازبینی، خروج‌های
> `0`/`1` و خروجی دامنه بی‌تغییرند. آزمون‌ها همه ساختگی: شمارش **۲۴/۲۴**، Cohort ‏**۲۰/۲۰**، مقایسه‌گر **۲۲/۲۲**؛ شش فایل
> آزمون کمپین **۱۲۱/۱۲۱** (مبنا ۱۰۸/۱۰۸)؛ ۲۵/۲۵ جهش دستی شکار شد؛ اجرای Spawn‌شده و اجرای دستی `pnpm run` روی Windows
> با ۵۹ مسیر بلند (حدود ۱۱٬۳۰۰ نویسه اگر یک‌به‌یک) موفقیت، شکست دامنه‌ای و خروج `2` را بی‌تغییر فایل و بی‌نشت نشان داد.
> Runbook § ۱۳ (L12، L13، ماتریس)، § ۱۷ سند آمادگی و ADR-055 به‌روز شدند. **هیچ گزارش، Manifest، شمارش یا بازبینی واقعی
> وجود ندارد.** سطر ۹ `UNVERIFIED`، سطر ۱۰ `BLOCKED`، سطر ۱۱ `UNVERIFIED`، سطرهای ۲/۶/۷/۸ حل‌نشده، **NO-GO**. هیچ
...
>
> **به‌روزرسانی 2026-09-17 (ADR-055 — Manifest مسیر Log شغل برای بازیابی Fallback؛ حکم NO-GO):**
> `recover:aggregation-campaign-logs` شکل `--logs-manifest <file>` گرفت (فقط‌خواندنی یا با `--output-dir` پیش یا پس از آن؛
> شکل صریح حفظ شد) تا ۵۹ مسیر بلند Log روی Windows از سقف ۸۱۹۱ نویسهٔ `cmd.exe` در `pnpm run` نگذرد. دستور زبان Manifest
> گزارش در `scripts/aggregation-campaign-report-manifest-lib.mjs` به Primitive کراندار `parsePathManifest`/`loadPathManifest`
> تبدیل شد (Exportهای گزارش، دقیقاً ۵۹، Wrapper بی‌تغییرند؛ سه مجموعهٔ آن **۶۶/۶۶**). Manifest ‏Log: ۱ تا ۱۲۸ خط
> (`LOG_RECOVERY_LIMITS.maxLogs`)، حداکثر ۱۳۱٬۲۰۰ بایت، خط نسبی نسبت به پوشهٔ Manifest، ترتیب خط همان ترتیب ورودی. خطای
> استفاده/Manifest و مسیر تکراری پس از resolve (هر دو شکل) خروج `2` با پیام ثابت، پیش از هر خواندن Log و هر فراخوان خروجی؛
> شکل صریح اکنون مسیر خالی/بیش از ۱۰۲۴ بایت، بیش از ۱۲۸ مسیر، `--` دیرهنگام و مسیر تکراری را هم با خروج `2` رد می‌کند. Log
> فهرست‌شدهٔ ناخوانا/بزرگ همچنان خروج `1`؛ خروجی، شمارش، پیوند Slot و نوشتن همه‌یا‌هیچ بی‌تغییر. آزمون‌ها ساختگی: بازیابی
> **۳۳/۳۳** با اجرای Spawn‌شدهٔ واقعی `pnpm run` (۵۹ مسیر ۲۱۳ نویسه‌ای، ۱۲٬۷۴۴ نویسه اگر یک‌به‌یک، از یک Manifest کوتاه؛
> فقط‌خواندنی بی‌تغییر، CRLF خروج `2`، `--output-dir` فقط پوشهٔ تازه و ۵۹ گزارش)؛ تمرین پس از اجرا اکنون بازیابی را از
> `pnpm` و `--logs-manifest` اجرا می‌کند (**۵/۵**)؛ هفت فایل آزمون کمپین **۱۳۳/۱۳۳**؛ ۱۵/۱۵ جهش شکار شد. Runbook ‏§ ۱۹ سند
> آمادگی، یادداشت L11 در § ۱۳ و ADR-055 به‌روز شدند. فقط شکاف انتقال آفلاین Windows بسته شد؛ Manifest انتقال اپراتور است،
> نه شاهد. **هیچ شاهد زنده‌ای وجود ندارد.** سطر ۹ `UNVERIFIED`، سطر ۱۰ `BLOCKED`، سطر ۱۱ `UNVERIFIED`، سطرهای ۲/۶/۷/۸
> حل‌نشده، **NO-GO**. هیچ Workflow، کمپین، دانلود، فراخوان GitHub/شبکه، `ci.yml`، فاز تست، آستانه یا Backlog تغییر نکرد.
> `COM-009` همچنان `READY`/۱۳، ADR-053 و ADR-055 همچنان `Proposed`؛ AUD-004 باز است.
>
> **به‌روزرسانی 2026-09-17 (ADR-055 — کارت فرمان L11–L13؛ حکم NO-GO):** پیوست § ۲۰ سند آمادگی چهار فرمان Manifestمحور
> `pnpm run` بازیابی Fallback، مقایسهٔ اختیاری، شمارش و بازبینی Cohort را به ترتیب Runbook با خطوط موفقیت، معنای خروج
> `1`/`2`، سه نوع ورودی (Manifest ‏Log ‏۱–۱۲۸، Manifest گزارش دقیقاً ۵۹، Snapshot ‏JSON) و الزام مسیر مطلق (`pnpm run` از
> ریشهٔ Repository اجرا می‌کند) در یک صفحه آورد؛ قاعدهٔ تازه‌ای نیست و § ۱۳ حاکم است. آزمون دستی ایستای
> `scripts/aggregation-campaign-command-card.test.mjs` ‏(**۵/۵**، بیرون از `pnpm verify`/فازها/CI) فرمان‌ها را با
> `package.json`، Parserها و متن خروجی واقعی می‌سنجد و Drift (اسکریپت/گزینه/ترتیب/آرگومان/نقش جابه‌جا) را رد می‌کند؛ ۱۱/۱۱ جهش
> شکار شد. کد تولیدی تغییر نکرد و ناسازگاری قراردادی یافت نشد. **هیچ شاهد زنده‌ای وجود ندارد**؛ سطرهای ۹/۱۰/۱۱ و ۲/۶/۷/۸
> بی‌تغییر، **NO-GO**. AUD-004 باز است.
>
> **اصلاح 2026-09-17 (ADR-055 — نقل‌قول مسیر در کارت فرمان L11–L13؛ حکم NO-GO):** Placeholderهای مسیر در چهار فرمان
> کارت § ۲۰ و فرمان‌های توصیه‌شدهٔ § ۱۳ L11–L13 بی‌نقل‌قول بودند و مسیر مطلق دارای فاصله پیش از Parser شکسته می‌شد؛ آزمون
> ایستا با `split(' ')` و مسیرهای بی‌فاصله آن را پنهان می‌کرد. اکنون همه در Double Quote‌اند (با قاعدهٔ حفظ نقل‌قول و منع `\`
> پایانی در § ۲۰.۱) و آزمون دستی کارت (**۸/۸**) با خوانندهٔ سخت‌گیر دستور ثابت، مسیرهای دارای فاصله تا ۱۰۲۴ بایت، Drift
> نقل‌قول و اجرای واقعی `cmd.exe`+`pnpm` روی Windows (خروج `2` قرارداد Manifest، بدون نوشتن) آن را می‌سنجد؛ § ۲۱ سند آمادگی.
> فقط اصلاح متن و نقطهٔ کور آزمون؛ **هیچ شاهد زنده‌ای نیست**؛ سطرها و **NO-GO** بی‌تغییر؛ `COM-009` همچنان `READY`/۱۳؛
> ADR-053 و ADR-055 `Proposed`؛ AUD-004 باز است.
>
> **اصلاح بعدی 2026-09-17 (ADR-055 — سخت‌سازی Harness آزمون کارت؛ حکم NO-GO):** آزمون Smoke ویندوز به `pnpm.cmd` سراسری
> روی PATH وابسته بود و بدون آن (با Corepack سالم) ۷/۸ می‌شد و خطای Runner را خطای قرارداد Manifest نشان می‌داد. اکنون
> Preflight مدیر بسته (`pnpm` مستقیم، وگرنه Shim `pnpm.cmd` متعلق به آزمون روی Launcher ‏Corepack کنار Node فعال، با PATH
> فقط برای فرزند و تطبیق نسخهٔ `packageManager`) پیش از هر فرمان اجرا می‌شود و خرابی Runner با جملهٔ ثابت و بدون مسیر جدا
> می‌شود؛ آزمون دستی کارت **۱۰/۱۰** در هر دو محیط (مستقیم و Corepack). فقط Harness آزمون؛ **هیچ شاهد زنده‌ای نیست**؛ سطرها و
> **NO-GO** بی‌تغییر؛ AUD-004 باز است.
>
> **ممیزی پذیرش 2026-09-17 (COM-009 / AUD-004 — فقط سند؛ ADR-055 متوقف):** ماتریس معیاربه‌معیار در
> [`docs/evidence/com-009/aud-004-acceptance-audit-2026-09-17.md`](docs/evidence/com-009/aud-004-acceptance-audit-2026-09-17.md).
> **حکم (A):** یک مانع درون‌دامنه — identity ردِ `403 FORBIDDEN` فراخوان سرویس (AuthGuard) را ثبت نمی‌کند، در حالی که
> ADR-053 § ۴ `FORBIDDEN` را در دامنهٔ MVP می‌آورد؛ گام بعدی کوچک: **Phase C11** با هدف آزمون شکست‌خورده در
> `security-event-auth-guard.int-spec.ts` (یا تصمیم تاریخ‌دار مالک محصول برای انتقال به R-2). بقیهٔ ردیف‌های AUD-004 با CI
> `35222180680` روی `03b992d` اثبات اجرایی دارند. باز/نیازمند تصمیم: خوانش «هر عمل» COM-009 با R-1، `SERVICE_TENANT_CONTEXT_INVALID`
> (در identity دست‌نیافتنی)، نیمهٔ دوم سناریوی E2E حسابرسی (§ ۶٫۶)، Trivy پس از Merge، پوشش ۹۰٪ identity (دروازهٔ CI ندارد)،
> OpenAPI ثبت‌شدهٔ identity، صادرات، Purge، Alertmanager، Scrape واقعی و احراز هویت Broker. قلم Backlog وعده‌داده‌شدهٔ R-2
...
### به‌روزرسانی 2026-09-21 — از اعلان تا نخستین صفحهٔ عملیاتی

`main` روی `6ea1260` است و **هیچ PR بازی نمانده**. از به‌روزرسانی 2026-09-18
تاکنون هفده PR نشسته‌اند (#51 تا #67):

| PR  | کار                                                                | Merge شد در |
| --- | ------------------------------------------------------------------ | ----------- |
| #54 | `EXP-001` الف — Workspace `apps/web` و زنجیرهٔ کیفیت، `ADR-058`    | `8c04477`   |
| #55 | `EXP-001` ب — Design Token، فونت محلی و لایهٔ ارائهٔ فارسی         | `4b15725`   |
| #56 | `EXP-001` ج — کتابخانهٔ Component و سه حالت اجباری                 | `e7b0372`   |
| #57 | `D-011` — رویداد تغییر برنامهٔ نگهداری برای ردپای حسابرسی          | `18946a2`   |
| #58 | انتشار رویدادهای خواندن و Dismiss اعلان از Outbox                  | `419afde`   |
| #60 | `NTF-003` — ترجیحات، نردبان تقدم پنج‌لایه، بستن `Q-38`             | `e6d509e`   |
| #62 | `pnpm verify` بدون پایگاه داده اجراشدنی شد (`test` فقط `unit`)     | `f7755e0`   |
| #63 | `Q-37` نیمهٔ مهندسی — Port `MailChannel` و Adapter SMTP            | `bcd3cd5`   |
| #64 | برداشتن دروازهٔ کهنهٔ «UI فقط با تأیید» و هم‌خوانی اسناد با مخزن   | `df54a43`   |
| #65 | `NTF-004` — کانال ایمیل کامل، ساعات سکوت، هشدار `DEAD`             | `db39f30`   |
| #66 | `EXP-002` الف — نشست سمت سرور (`ADR-059`)، `/login` و داشبورد      | `3cf79aa`   |
| #67 | `EXP-002` ب — فهرست ماشین‌آلات و پروندهٔ الکترونیکی؛ رفع نشت Relay | `6ea1260`   |

(#51، #52، #53، #59، #61 به‌روزرسانی‌های مستندات و فرایند بودند.)

**سه نقص واقعی که فقط اجرای واقعی گرفت، نه بازبینی:** (۱) نشت Producer کافکا در
خاموشی که هر اجرای CI را بیست دقیقه Hang می‌کرد در حالی که همهٔ تست‌ها سبز بودند
(PR #60)؛ (۲) طول برچسب اصالت GCM که Semgrep گرفت (PR #66)؛ (۳) Relay صندوق
خروجی که در Suite های API خاموش نمی‌شد و اجرا را **پس از قبولی همهٔ ۱۳ Suite**
قرمز می‌کرد (PR #67). هر سه در بخش ۲۲ و ۳۰ ثبت‌اند.

**فاز:** پیاده‌سازی MVP. **۲۷ سند معماری و ۵۹ ADR.** دوازده سرویس Backend روی
`main`؛ پورتال `apps/web` حالا ورود، داشبورد و دو صفحهٔ دارایی دارد. پنج سرویس
هنوز ساخته نشده‌اند (بخش ۲۷).

> **به‌روزرسانی 2026-09-23 — راننده و تخصیص، دومین Consumer زیرساخت نوشتن.**
> `/drivers` (فهرست + ثبت) و `/drivers/[id]` (جزئیات + ویرایش + تغییر وضعیت +
> تخصیص/پایان تخصیص) روی همان زیرساخت نوشتن ساخته شدند که `/usage` بنا کرد —
> پنج فرم، نه یک، و یک شکاف واقعی زیرساخت را باز کرد: `writeThroughGateway`
> فقط `POST` می‌فرستاد، ولی ویرایش راننده `PATCH /v1/drivers/{id}` است.
> `WriteCall` یک `method` اختیاری گرفت (پیش‌فرض `POST`، پس فراخوانی `/usage`
> عوض نشد). شناسهٔ راننده/تخصیص به هر Action با `action.bind(null, id)`
> می‌رسد، نه فیلد پنهان فرم — چیزی که فرم جمع نکرده، فیلد فرم نیست.
> `EXP-002` همچنان `IN_PROGRESS`؛ تنها سازمان و اعضا مانده که هم‌زمان توسط
> نشست دیگری ساخته می‌شود.
>
> **یک نقص واقعی که خودِ تست گرفت.** فرم تغییر وضعیت فقط وضعیت فعلی را کنار
> می‌گذاشت، نه گذارهای غیرمجاز را — برای رانندهٔ از‌رده‌خارج (وضعیت پایانی،
> بدون گذار مجاز) همچنان `فعال` و `معلق` را پیشنهاد می‌داد. جدول گذار
> `driver-lifecycle.ts` در سمت کلاینت هم تکرار شد (فقط برای UX، طبق `docs/16
§ ۱۶٫۱۱` — سرویس همچنان مرجع اجراست).
>
> **یک مرز که Build اثباتش کرد:** Component سمت کلاینت نمی‌تواند از
> `@/server/*` چیزی Import کند — `node:crypto` وارد Bundle مرورگر می‌شد و
> `next build` رد کرد. نام فیلدها و شکل فرم به `src/lib/` رفتند.

> **به‌روزرسانی 2026-09-22 — پورتال حالا می‌نویسد.** زیرساخت نوشتن ساخته شد و
> `/usage` نخستین صفحه‌ای است که از آن استفاده می‌کند: توکن CSRF گره‌خورده به
> نشست مهرشده که **پیش از** هر تماس با Gateway بررسی می‌شود، شناسهٔ ارسال
> ساخته‌شده روی سرور که هم به‌صورت `Idempotency-Key` و هم `clientReference`
> می‌رود (پس دوبار کلیک‌کردن یک رکورد است، نه دو تا)، نگاشت جزئیات اعتبارسنجی
> سرویس روی همان فیلدی که علتش بوده، و یک Form ساده روی Server Action که بدون
> جاوااسکریپت هم کار می‌کند. `EXP-002` همچنان `IN_PROGRESS` و صفر امتیاز است.

> **به‌روزرسانی 2026-09-23 — یک ارتقای سطح دسترسی زنده روی `main` بسته شد.**
> هر `ORGANIZATION_ADMIN` می‌توانست با **یک درخواست**، روی عضویت خودش،
> نقش `SYSTEM_ADMIN` بگیرد: `updateMembershipRoles` آرایهٔ درخواستی را
> مستقیم روی ردیف می‌نوشت، `roleSchema` هر ۱۱ نقش را می‌پذیرفت، و
> `@Roles(...)` فقط می‌گفت چه کسی اجازهٔ تغییر نقش دارد — نه چه نقشی را.
> هر بررسی Tenant هم پاس می‌شد، چون عضویت در سازمان خودش بود.
>
> این **سیاست تازه نبود**: `docs/09` تهدید `I2` را نوشته و کنترلش را
> سه‌بخشی خواسته بود و بخش سوم — «تست مجوزدهی» — هرگز نوشته نشده بود.
> ستون **دامنه** همان جدول اکنون به‌عنوان داده در
> `identity/role-grants.ts` زندگی می‌کند و سه متغیر `ROLE_GRANTS_BY_*`
> آن را پیکربندی‌پذیر می‌کنند (`docs/24` **Q-60**).
>
> **چهار ورودی نقش می‌نویسند، نه یکی.** `createUser`، `addMembership`،
> `updateMembershipRoles` و — کم‌صداترین آن‌ها — `approveRegistration`،
> که `dto.roles ?? request.requestedRoles` را اعطا می‌کرد: یعنی یک متقاضی
> ناشناس روی Endpoint عمومی ثبت‌نام می‌توانست `SYSTEM_ADMIN` بخواهد و
> منتظر یک کلیک تأیید بماند. قاعده متقارن است: عضویتی را فقط
> می‌توان اداره یا باطل کرد که نقش‌هایش در توان اعطای همان فرد باشد،
> وگرنه مدیر سازمان می‌توانست اپراتور پلتفرم را در سازمان خودش
> تنزل دهد یا اخراج کند.
>
> **شواهد:** `role-escalation.spec.ts` همان حمله را از روی HTTP اجرا می‌کند —
> Token واقعی، `AuthGuard` و `RolesGuard` واقعی، Controller و Service واقعی — و
> هر رد را با «هیچ چیزی نوشته نشد» می‌سنجد. ۳۰ آزمون حمله و مسیر مشروع،
> ۲۴ آزمون سلسله‌مراتب، ۹ آزمون پیکربندی.
>
> **آنچه هنوز نیست:** این ردهای تازه جزء ده محل رد الگوی AUD-004
> نیستند، پس به Audit نمی‌روند — مثل هر `403` دیگری که هنوز
> Instrument نشده (`ADR-053` طرح § ۸، R-2). یک تلاش نافرجام برای ارتقای
> سطح دسترسی دقیقاً همان چیزی است که باید دیده شود؛ کار بعدی.

> **به‌روزرسانی 2026-09-23 (دوم) — `/organizations`: سازمان و اعضا.**
> مشخصات سازمان (همان سه فیلدی که `updateOrganizationSchema` می‌پذیرد)، فهرست
> اعضا، نقش‌های هر عضو، و ابطال عضویت. مدیریت سلسله‌مراتب عمداً نیست:
> `docs/16` § ۱۶٫۶ آن را به `/organizations` دیگری در `apps/admin` برای `UNION_ADMIN`
> سپرده، و `organization-service` هم همین را می‌گوید — `:id/move`، `:id/status` و
> `:id/policies` هر سه `@Roles('UNION_ADMIN')` دارند.
>
> **نقطهٔ اصلی: انتخابگر نقش دروغ نمی‌گوید.** پس از PR #77، سلسله‌مراتب اعطای
> نقش پیکربندی است، نه ثابت. پس فهرستی Hard-Code‌شده در پورتال، همان لحظه که
> کسی `ROLE_GRANTS_BY_*` را عوض کند، با سرویس اختلاف پیدا می‌کرد — یا نقشی را
> پیشنهاد می‌داد که رد می‌شود، یا نقشی را پنهان می‌کرد که مجاز بود. پس
> `identity-service` حالا روی `GET /v1/users/me` یک فیلد `grantableRoles` می‌دهد که
> مستقیم از `grantableRoles()` همان `role-grants.ts` می‌آید — نه پیاده‌سازی دوم. این
> یک کنترل امنیتی نیست (`docs/16` § ۱۶٫۱۱)؛ سرویس هر چه فرم بفرستد را خودش
> می‌سنجد. این فقط فرمی است که به کاربر دروغ نمی‌گوید.
>
> نقشی که عضو دارد و مدیر نمی‌تواند اعطا کند، به‌صورت Hidden Input می‌ماند
> و روی صفحه نام برده می‌شود — چون سرویس **کل مجموعهٔ حاصل** را می‌سنجد و
> حذفش از فرم، درخواست حذف آن نقش خوانده می‌شد.
>
> **یک نقص واقعی در کد مشترک پیدا شد:** `callGateway` همیشه پاسخ را JSON
> پارس می‌کرد، پس `204 No Content` — که دقیقاً پاسخ
> `POST /v1/memberships/:id/revoke` است — استثنا می‌داد **پس از** آنکه سرویس
> کارش را انجام داده بود: عضویت باطل، و صفحه می‌گوید نشد. رفع شد و سه آزمون
> گرفت.
>
> **شواهد:** `apps/web` واحد **۴۰ Suite و ۶۶۶ آزمون** (۶۲۱ پیش‌تر)؛ مرورگر
> **۱۰ آزمون** (۶ پیش‌تر) روی Build تولید؛ `identity-service` **۸۵۷ آزمون**.
> `/organizations` یک مسیر Dynamic به اندازهٔ 2.62kB است.

> **به‌روزرسانی 2026-09-24 — سه رفع کوچک مستقل از ADR-060.** (۱) `getUser` با شکست بسته می‌شود: توکنی که
> سازمانی حل نمی‌کند، برای غیرخود `404` می‌گیرد — پیش‌تر بررسی عضویت را کلاً جا می‌انداخت و `findUserById` بی‌Tenant
> برای هر کسی پاسخ می‌داد. (۲) ویرایش پروفایل **دیگری** فقط با `ORGANIZATION_ADMIN` (یا `SYSTEM_ADMIN`)؛ پیش‌تر هر
> `OPERATOR` نام و تلفن همکارش را عوض می‌کرد. تا گارد ADR-060 نیاید دقیق نیست — نقش‌ها هنوز سراسری‌اند — ولی بررسی
> عضویت همچنان مدیر را به افراد سازمانی که برایش عمل می‌کند محدود نگه می‌دارد. (۳) `exp` در هر سه Verifier الزامی
> است؛ jose آن را فقط اگر باشد می‌سنجید و توکن بی‌`exp` هرگز منقضی نمی‌شد. Verifier توکن کاربر نخستین Spec خودش
> را گرفت، با JWKS واقعی که از یک سرور HTTP محلی سرو می‌شود.
>
> **به‌روزرسانی 2026-09-24 — `/orders`: نخستین صفحهٔ `EXP-003`، و یک Stepper که دروغ نمی‌گوید.**
> فهرست سفارش (یک زبانه برای هر سمت، چون یک سازمان در یک سفارش خریدار و در دیگری فروشنده
> است) و جزئیات سفارش با Stepper و فرمان‌هایی که **همین بیننده** می‌تواند بدهد.
>
> **فرمان‌ها از سرویس می‌آیند، نه از پورتال.** `OrderView.availableActions` را
> `marketplace-service` برای هر فراخواننده از سه منبع می‌سازد: جدول انتقال، محدودسازی
> `from` هر فرمان، و `access.ts`. استخراجش در پورتال **از روز اول غلط** بود، نه فقط
> شکننده: جدول انتقال `DISPUTED → RECEIPT_CONFIRMED` را دارد — برای اپراتوری که اختلاف را
> حل می‌کند — و خواندن جدول به‌تنهایی دکمهٔ «تأیید دریافت» را روی سفارش **در اختلاف** به
> خریدار نشان می‌داد.
>
> **وجه پیش از پذیرش فروشنده نگه داشته می‌شود** و Stepper آن را مرحلهٔ جدا نشان می‌دهد.
> بیرون از مسیر عادی (اختلاف، لغو، ناکامی) هر مرحله فقط وقتی «انجام‌شده» است که سفارش
> مُهر زمانی آن را دارد؛ بقیه «به آن نرسید» است، نه «پیش رو».
>
> **دو نقص در `marketplace-service` پیدا و رفع شد:** (۱) **D-032** — فرمان‌ها به سازمان
> بیگانه `403` می‌دادند، نه `404`، و وجود سفارش را لو می‌دادند؛ یک آزمون موجود نامش «۴۰۴»
> بود و `403` را تأیید می‌کرد. (۲) `load()`، خوانش پس از هر فرمان، `review` را انتخاب
> نمی‌کرد و `availableActions` نظر دوم را پیشنهاد می‌داد؛ حالا فیلد در نوع ردیف **اجباری**
> است تا Query فراموش‌کار Typecheck نشود.
>
> **شواهد:** `apps/web` ۴۵ Suite و ۷۴۸ آزمون؛ `marketplace-service` ۳۱۸ آزمون واحد؛ یک
> Spec یکپارچگی تازه که نشان می‌دهد هر فرمان پیشنهادشده واقعاً پذیرفته می‌شود و فرمان
> پیشنهادنشده واقعاً رد می‌شود. **Docker روی این دستگاه خاموش بود**، پس آزمون‌های
> یکپارچگی فقط Typecheck شده‌اند و CI نخستین اجرای واقعی آن‌هاست. Spec مرورگر برای
> `/orders` عمداً نوشته نشد: یک نشست ابری روی `apps/web/e2e` و پیکربندی کار می‌کند.
>
> **در بازبینی PM پیدا شد و رفع شد:** `cancel()` هیچ `from`ی نداشت و گذار `DISPUTED → CANCELLING` جدول را به ارث
> می‌برد — گذاری که ADR-038 فقط به `ResolveDispute(REFUND)` اپراتور داده است. پس خریدار می‌توانست اختلافی را که خودش ثبت
> کرده با لغو ترک کند، Saga وجه را برگرداند، و فروشنده‌ای که تحویل داده بود بی‌پول بماند. همان شکل دام «تأیید دریافت»،
> روی درِ دوم خروج از اختلاف. آزمون‌های خود این شاخه همین نقص را **تأیید** می‌کردند (`DISPUTED → ['CANCEL']`) و اصلاح شدند.
>
> **فاصلهٔ شناخته‌شدهٔ ADR و کد، بیرون از دامنهٔ این PR:** ADR-038 در سطرهای ۶۲ و ۶۵ لغو از `FUNDS_HELD` و `CONFIRMED`
> را به «خریدار / تأمین‌کننده» می‌دهد، ولی کد فقط خریدار را می‌پذیرد (`assertBuyer`). یکی از دو باید عوض شود؛ این PR
> هیچ‌کدام را عوض نمی‌کند.

**گام اجرایی بعدی:** بقیهٔ `EXP-002` — فقط سازمان و اعضا مانده، که هم‌زمان با
این کار توسط نشست دیگری ساخته می‌شود. پس از آن `EXP-003` تا `EXP-005`. ترتیب
همچنان از `planning/backlog.json` خوانده می‌شود.

---

### به‌روزرسانی 2026-09-18 — شش Merge در یک روز، و پایان کار موازی C0..C3

`main` روی `f7252d3` است و **هیچ PR بازی نمانده**. اجرای push روی همین Commit،
`35364991343`، هر شش Job را سبز کرد.

| PR  | کار                                                   | Merge شد در |
| --- | ----------------------------------------------------- | ----------- |
| #44 | `AUD-004` — قرارداد `audit-service` (فاز C11)         | `9fa75cc`   |
| #45 | `ADR-056` — مرز لجستیک رو‌به‌جلوی `inventory`         | `f2eb8fa`   |
| #46 | `NTF-001` — اعلان درون‌برنامه‌ای با Dedupe محتوامحور  | `cc89660`   |
| #47 | `NTF-002` — API خواندن و وضعیت خوانده‌شدهٔ یک‌بارنوشت | `7389ffd`   |
| #48 | `COM-005` — فاز ۲ گام ۱ `ADR-052`، سیگنال‌های عملکرد  | `f7252d3`   |
| #49 | `docs/26-branch-registry.md` — هماهنگی کار موازی      | `76feb8e`   |
| #50 | رفع نقض `ck_dedupe_window_ordered` زیر همروندی        | `78ac7b0`   |

**دو نقص واقعی که فقط زیر زیرساخت واقعی و فشار همروندی بیرون آمدند** (بخش ۲۲):
یک Race در پنجرهٔ Dedupe اعلان، و یک `CHECK` که یک Migration بدون Backfill
نقض می‌کرد. هیچ‌کدام را بازبینی کد نگرفت؛ هر دو را اجرای واقعی گرفت.
...
### به‌روزرسانی 2026-09-18 — شمارش واقعی از اجرای سبز `35364991343` (`f7252d3`)

اعداد زیر از Log همان اجرا برداشته شده‌اند، نه از حافظه و نه از اجرای محلی.

| بخش                    | تست واحد  | تست یکپارچگی |
| ---------------------- | --------- | ------------ |
| `identity-service`     | ۷۸۹       | ۹۵           |
| `audit-service`        | ۷۱۷       | ۱۷۷          |
| `economic-service`     | ۴۲۵       | ۲۹۴          |
...
| `api-gateway`          | ۹۲        | —            |
| `fleet-service`        | ۹۱        | ۳۶           |
| `organization-service` | ۲۱        | ۱            |
| شش Package مشترک       | ۳۵۴       | —            |
| **مجموع**              | **۳٬۸۶۹** | **۱٬۱۷۶**    |

**E2E:** ۹۵ سناریوی Playwright، همه سبز روی همان اجرا (۲٫۶ دقیقه، یک Worker).

**آنچه این جدول می‌گوید و آنچه نمی‌گوید.** این‌ها `CI VERIFIED` هستند: روی
Runner واقعی GitHub Actions اجرا شدند و سبز بودند. این با **پذیرش** یکی نیست —
`COM-008` با وجود سبزبودن کامل، یک مسدودکنندهٔ پذیرش باز دارد (انحراف حسابرسی
`NTF-002`، بخش ۲۲). سبزی CI و پذیرش محصول دو ستون جدا هستند و هرگز نباید در
یک خانه نوشته شوند.

**دربارهٔ Frontend — به‌روزرسانی دوم 2026-09-21.** یادداشت زیر برای صبح همین
روز درست بود و تا عصر کهنه شد: پورتال دیگر فقط پایه نیست. `EXP-002` با PR #66 و
#67 نشست — ورود با نشست سمت سرور (`ADR-059`)، داشبورد، فهرست ماشین‌آلات و پروندهٔ
الکترونیکی دارایی — و **نخستین Job مرورگری مخزن** (`Portal in a browser`) حالا
در CI اجرا می‌شود. شمارش امروز: ۳۷۸ تست واحد و سه تست مرورگری در برابر Build
تولیدی. ردیف جدول پایین‌تر مطابق همین به‌روز شد.
...
| **پورتال کاربر (`apps/web`) — ورود، داشبورد، دارایی، نگهداری و ثبت کارکرد** |                          ✅                           | ✅ (۴۸۰ واحد + ۶ مرورگری)  | ⚠️ **CI VERIFIED**، نه زنده: Job ‏`Portal in a browser` در برابر Build تولیدی سبز است؛ هیچ صفحه‌ای — از جمله نخستین مسیر نوشتن (`/usage`) — هرگز در برابر Keycloak و Gateway زنده اجرا نشده و Job مرورگری امروز هیچ‌کدام را بالا نمی‌آورد (`ADR-059`)                      |
...
apps/
  web/    IMPLEMENTED — Design System، قالب‌بندی فارسی/جلالی، نشست سمت سرور
          (ADR-059)، /login، داشبورد، /assets و /assets/[id]. ۳۷۸ تست
          واحد + ۳ تست مرورگری در CI. بقیهٔ EXP-002 ساخته نشده
...
tests/
  e2e/    IMPLEMENTED — @rasta/e2e: Playwright Config، Global Setup،
          ...
          در `apps/web` زندگی می‌کند و از EXP-002 در CI اجرا می‌شود)

infrastructure/
  docker/   Postgres Init، Kafka Topics، Keycloak Realm — IMPLEMENTED
  k8s/      NOT_STARTED — پوشه خالی

docs/       ۲۷ سند + ۵۹ ADR + events/api/database/security/deployment/runbooks
scripts/
...
### به‌روزرسانی 2026-09-18 — جدول بالا کهنه است

ردیف «۸ سرویس دیگر NOT_STARTED» دیگر درست نیست. **دوازده سرویس** روی `main`
...
| **نگهداشت توکن در پورتال (`ADR-059`)** |     ✅      |   ✅   |                    ⚠️ فقط CI                    | توکن هرگز به مرورگر نمی‌رسد: Authorization Code + PKCE روی سرور، نشست در Cookie مهرشده با AES-256-GCM (`HttpOnly`، `Secure`، `SameSite=Strict`)، توکن CSRF روی خروج، `authTagLength` صریح (یافتهٔ Semgrep). هرگز در برابر Keycloak زنده اجرا نشده                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- 🆕 **D-008 باز است** — سه قاعده Supply-Chain که Lockfile فعلی رد می‌کند.
  **به‌روزرسانی 2026-09-22:** ✅ بسته شد — هر سه قاعده فعال، چهار هشدار Trust
  بررسی و به‌نام و به‌نسخه استثنا شدند (`docs/23` D-008).
...
`main` روی `7389ffd` قرمز شد. `concurrency.int-spec.ts` در `notification-service`
با SQLSTATE `23514` می‌افتاد: نقض `ck_dedupe_window_ordered`، با سطری که
`first_seen_at = …25.13643` و `last_seen_at = …25.100399` داشت — یعنی «آخرین»
پیش از «نخستین».

**ریشه.** `now()` در PostgreSQL همان `transaction_timestamp()` است: لحظهٔ **شروع
تراکنش**، ثابت در کل عمر آن، و بی‌ربط به زمان اجرای دستور. تراکنش‌های متعارض
روی `dedupe_key` به ترتیب **Commit** سریال می‌شوند، نه به ترتیب شروع. پس
تراکنشی که زودتر شروع شده می‌تواند پشت تراکنشی که دیرتر شروع شده بلوک شود و
بعد در شاخهٔ `DO UPDATE` مقدار `last_seen_at` را با ساعتی بنویسد که از
`first_seen_at` همان سطر عقب‌تر است. شرط پایگاه داده کارش را درست کرد؛ کد یک
سطر ناممکن به آن داد.

**رفع** (PR #50، `78ac7b0`):
`"last_seen_at" = GREATEST("notification_dedupe"."last_seen_at", now())`.

`clock_timestamp()` نامزد دیگر بود و **رد شد**، به دو دلیل که هر دو ارزش ثبت
دارند: نخست، `GREATEST` نامتغیر را **به‌صورت ساختاری** برقرار می‌کند بدون هیچ
فرضی دربارهٔ رو‌به‌جلو بودن ساعت، پس یک پرش NTP به عقب هم آن را نمی‌شکند. دوم،
`clock_timestamp()` تابعی `volatile` است و چهار آزمون پنجرهٔ `expires_at <= now()`
در همان **یک** دستور هرکدام در لحظه‌ای متفاوت ارزیابی می‌شدند و می‌توانستند با
هم اختلاف کنند — مثلاً `seen_count` را ریست کنند بدون ریست‌کردن `first_seen_at`.
آن نقض، نامرئی می‌بود؛ نقض فعلی دست‌کم صدا داشت.

**آنچه رفع نشد و عمداً باز مانده:** `expires_at` از ساعت **برنامه**
(`Date.now()`) می‌آید در حالی که آزمون پنجره و نیمهٔ دوم شرط از ساعت
**پایگاه داده** می‌خوانند. نقضش به یک شبانه‌روز عقب‌ماندگی برنامه نیاز دارد، چون
`NOTIFICATION_DEDUPE_RETENTION_DAYS` صحیحِ دست‌کم ۱ است. محاسبه‌اش در SQL این
جفت‌شدگی را برمی‌دارد و **تغییر و تست خودش را می‌خواهد**؛ کنار همان دستور
مستند شده تا به‌عنوان رفع جانبی قاچاقی وارد نشود.

**تست.** بازتولید، مسابقه نمی‌زند؛ ترتیب را تحمیل می‌کند: ingest دیرآمده پشت یک
سطر `processed_event` کامیت‌نشده پین می‌شود، پس تراکنشش شروع شده و `now()`اش
قفل شده در حالی که ingest دیگر پنجره را باز می‌کند و Commit می‌شود. روی کد
رفع‌نشده هر بار `23514` می‌دهد.

### D-029 — یک ادعای تست که یک‌شانزدهم مواقع اشتباه بود — رفع شد (2026-09-18) ✅

در `constraints.int-spec.ts`، تستِ «کلید خارجی تا Commit معوق است» کلید غایبش
را با `key.replace(/.$/, 'f')` می‌ساخت. وقتی `key` — که `sha256(ulid())` است —
خودش به `f` ختم شود، آن عبارت **همان کلید** را برمی‌گرداند، پس درج به ایندکس
یکتا می‌خورد (`23505`) و کلید خارجی هرگز ارزیابی نمی‌شود. یک رقم شانزدهی از
هر شانزده.

رفع: `key.slice(0, -1) + (key.endsWith('0') ? '1' : '0')`، به‌علاوهٔ دو گارد
(`not.toBe` و `toHaveLength`) تا ویرایش بعدی روی یک ادعای نام‌دار بیفتد نه روی
یک `23505` خام.

> **چرا این دو با هم مهم‌اند.** D-029 «ناپایدار» نبود و «رگرسیونِ رفعِ D-028»
> هم نبود. مرحلهٔ `Notification coverage gate` که آن Suite را بار دوم اجرا
> می‌کند، **پس از** `Integration tests` در همان Job است؛ تا وقتی D-028 می‌افتاد،
> Job پیش از رسیدن به آن مرحله متوقف می‌شد و D-029 اصلاً اجرا نمی‌شد. رفع
> اولی، همان چیزی بود که دومی را آشکار کرد.
>
> **قاعده‌ای که از این درآمد:** وقتی یک Job روی نخستین شکستش متوقف می‌شود،
> سبزشدن آن شکست خبر نمی‌دهد که بقیه سبزند — فقط خبر می‌دهد که حالا اجرا
> می‌شوند. در `docs/26-branch-registry.md § ۲۶٫۴` ثبت شد.

### D-030 — Migration فاز ۲ `ADR-052` بدون Backfill قید می‌گذاشت — رفع شد (2026-09-18) ✅

Migration سیگنال‌های عملکرد در `marketplace-service` یک `CHECK` روی ستون علت
لغو می‌گذاشت بدون آنکه سطرهای موجود را پر کند. روی ماشینی که پایگاه‌دادهٔ بقیهٔ
سرویس‌ها Migrate نشده بود هرگز دیده نمی‌شد، چون زنجیرهٔ `test:migration` آنجا
اصلاً کامل اجرا نمی‌شد. رفع در `0e4ab9f`، به‌علاوهٔ `dd7070c` که یک تست را که
فرض می‌کرد Outbox در یک Tick رله می‌شود به «تخلیهٔ کامل» تغییر داد.
...
- **Playwright نصب شد** (2026-08-29). **به‌روزرسانی 2026-09-21:** `apps/web`
  ...
  `pnpm --filter @rasta/web test:ui`). **از 2026-09-21 در CI است** (Job
  `Portal in a browser`، PR #66) و این بدهی بسته شد. پیش از آن: هنوز در CI نبود و این یک بدهی آگاهانه
...
### به‌روزرسانی 2026-09-20 — `Q-38` بسته شد
...
**`Q-38` — «آیا کاربر می‌تواند از یک اعلان الزامی انصراف دهد؟» — بسته شد**، با
همان تصمیم موقتی که `ADR-054 § ۵` از 2026-09-07 پیشنهاد کرده بود: `MANDATORY`
همیشه یک `IN_APP` می‌سازد که نمی‌شود از آن انصراف داد (فقط Dismiss)، و هر کانالی
که **فشار** می‌دهد زیر کنترل ترجیح می‌ماند. مدیر پروژه اختیارش را واگذار کرده
بود، با معیار «کیفیت و امنیت».

دو نکته که نباید گم شوند:

1. **پاسخ پیکربندی است، نه کد سخت.** به‌صورت `mandatoryChannels` به‌ازای هر
   قاعده در `rules.ts` پیاده شد، پس عوض‌کردنش نه Migration می‌خواهد نه تغییر
   دامنه — دقیقاً همان چیزی که «اثر پاسخ» پرسش وعده داده بود.
2. **امروز هیچ قاعده‌ای `MANDATORY` نیست** — هر سه `ROUTINE`اند. پس این سیاست در
   سرویس مستقر هیچ اثری ندارد و `422`اش از بیرون قابل‌دریافت نیست؛ به همین دلیل
   در قرارداد OpenAPI مستند **نشده** (استدلال `Q-07`). بستن پرسش یعنی قاعده‌اش
   معلوم و پیاده است، نه اینکه اعلان الزامی‌ای وجود دارد.

### به‌روزرسانی 2026-09-18 — شمارش فعلی روی `main`

منبع معتبر همیشه `docs/24-open-questions.md § ۲۴٫۱۰` است. امروز:
...
`Q-45` تا `Q-55` و `Q-57` از `AUD-004` آمدند، `Q-56` از فاز ۲ `ADR-052`، و
**`Q-44` بسته شد** با یک تصمیم محصول MVP.

> **`Q-44` دیگر قابل جابه‌جایی نیست.** `ADR-056 § ۲` آن را به‌عنوان مرجع پذیرش
> **کلمه‌به‌کلمه نقل کرده**؛ نقل‌قول را نمی‌شود ویرایش کرد بی‌آنکه سند دروغ
> شود. سه شاخه مستقلاً همین شماره را برداشته بودند و دوتای دیگر مجبور به
> بازشماری شدند. برای جلوگیری از تکرار، دفتر شماره‌های رزروشده در
> `docs/26-branch-registry.md § ۲۶٫۵` ساخته شد.
...
### به‌روزرسانی 2026-09-18 — ۵۷ ADR (`ADR-001` تا `ADR-057`)

`ADR-053` و `ADR-054` دیگر `Proposed` نیستند — هر دو پیاده شدند و روی `main`اند.
سه ADR تازه:

- **ADR-055 — قابلیت محیط فشار تجمیع.** با `AUD-004` آمد (PR #44)، شواهدش در
  `docs/evidence/adr-055/`.
- **ADR-056 — مرز لجستیک رو‌به‌جلوی `inventory-service`.** با PR #45 آمد و
  `Q-44` را با نقل‌قول مستقیم صاحب محصول بست.
- **ADR-057 — لنگرگذاری خارجی و بایگانی تغییرناپذیر Audit.** `Proposed`،
  سخت‌سازی پس از MVP. این سند ابتدا شمارهٔ `055` را برداشته بود که هم‌زمان روی
  `main` مصرف شد؛ بازشماری‌اش با همین تغییر انجام شد و پرسش بازش از `Q-44` به
  `Q-58` رفت، چون `Q-44` روی `main` بسته و در `ADR-056 § ۲` کلمه‌به‌کلمه نقل شده.
...
سه ADR که در آن زمان تازه بودند. از 2026-09-18، ADR-053 و ADR-054 هر دو پیاده و
روی `main`اند؛ فقط ADR-057 (که آن روز شمارهٔ ۰۵۵ را داشت) هنوز `Proposed` و
پیاده‌نشده است:
...
- **ADR-057 — لنگرگذاری خارجی و بایگانی تغییرناپذیر Audit (Proposed، سخت‌سازی پس از MVP).** Checkpointهای دسته‌ای زنجیرهٔ
  Hash با کلید نامتقارن غیرقابل‌Export در KMS/HSM امضا و در WORM/Object Lock داخل مرز اعتماد جدا نگه داشته می‌شوند. مسیر
  کسب‌وکار به KMS/WORM وابسته نمی‌شود؛ Lag به‌صورت `UNANCHORED` و Alert آشکار است. فروشنده، فاصلهٔ Checkpoint، Retention و
  مدل تفکیک اختیار در `Q-58` بازند. این تصمیم مقاومت و اثبات مستقل را بالا می‌برد، نه اینکه امنیت مطلق یا Tamper-proof بودن را
  ادعا کند.
...
### به‌روزرسانی 2026-09-21

- **`apps/web`** — `EXP-002` الف (PR #66، `3cf79aa`): ورود با `Authorization
Code + PKCE` در Route Handler های سرور، نشست در Cookie مهرشده، `/login`،
  داشبورد و خروج با توکن CSRF. تصمیمش `ADR-059` است و یک انحراف ثبت‌شده از
  `docs/16 § ۱۶٫۱` دارد: کتابخانهٔ `oidc-client-ts` به کار نرفت. نخستین Job
  مرورگری مخزن (`Portal in a browser`) با همین PR وارد CI شد.
- **`apps/web`** — `EXP-002` ب (PR #67، `6ea1260`): فهرست ماشین‌آلات با
  پالایه و صفحه‌بندی در URL، و پروندهٔ الکترونیکی دارایی. هر پاسخ سرویس از یک
  Schema می‌گذرد و فقط فیلدهای نمایشی از آن جان به در می‌برند؛ مختصات،
  ارجاع اسناد و جزئیات خام رویداد هرگز به درخت React نمی‌رسند.
- **`notification-service`** — رفع نشت Relay صندوق خروجی در Suite های API
  (PR #67): اجرای یکپارچگی پس از قبولی هر ۱۳ Suite با کد ۱ خارج می‌شد. ریشه با
  بستن عمدی پورت Mailpit بازتولید شد و با یک Relay بی‌اثر در
  `test/api-helpers.ts` بسته شد.
- **`notification-service`** — `NTF-004` (PR #65، `db39f30`)؛ جزئیاتش در
  بلوک پایین است.
...
- **`notification-service`** — `NTF-003`: ترجیحات اعلان به‌ازای مستأجر، نردبان
  ...
  ۲۰۸ واحد + ۱۱۰ یکپارچگی. (`ADR-054`؛ PR #60)
- **`docs/24-open-questions.md`** — **`Q-38` بسته شد**: اعلان الزامی همیشه
  `IN_APP` می‌سازد و از آن نمی‌شود انصراف داد؛ هر کانالی که **فشار** می‌دهد زیر
  کنترل ترجیح می‌ماند. پیاده به‌صورت `mandatoryChannels` به‌ازای هر قاعده، پس
  پاسخ بدون Migration عوض می‌شود. **هیچ قاعده‌ای امروز `MANDATORY` نیست**، پس
  سیاست در سرویس مستقر اثری ندارد و `422`اش در قرارداد مستند نشده (Q-07).
- **`notification-service`** — رفع نشت Producer کافکا در خاموشی، که آزمون‌های
  یکپارچگی این سرویس را در CI بیست دقیقه معلق می‌کرد در حالی که هر ۱۱۰ تست سبز
  بودند. شرح در § ۲۹.

### به‌روزرسانی 2026-09-18

- **`audit-service`** — `AUD-004`: قرارداد صریح `AUDIT_EVENT_RECORDED`، تجمیع
  ردها، و فاز C11 روی مسیرهای رد `AuthGuard`. ۷۱۷ واحد + ۱۷۷ یکپارچگی.
  (`ADR-053`، `ADR-055`؛ PR #44)
- **`notification-service`** — `NTF-001` اعلان درون‌برنامه‌ای با Dedupe
  محتوامحور و حل گیرنده از راه Token داخلی `SERVICE`؛ `NTF-002` API خواندن با
  وضعیت خوانده‌شدهٔ **یک‌بارنوشت در پایگاه داده**. ۱۸۰ واحد + ۷۰ یکپارچگی.
  (`ADR-054`؛ PR #46، #47)
- **`marketplace-service`** — فاز ۲ گام ۱ `ADR-052`: وعدهٔ تحویل، حل اختلاف با
  انتساب ساخت‌یافته، و علت ساخت‌یافتهٔ لغو. گام ۱-ت (کیفیت) عمداً ساخته **نشد** —
  `Q-56` باز است و پاسخش تصمیم محصول است. (PR #48)
- **`identity-service`** — نخستین Endpoint قابل‌فراخوانی توسط سرویس در تاریخ
  پلتفرم: `GET /v1/users` با `@AllowService('notification-service')`، دامنه‌محدود
  به `org_id` امضاشدهٔ توکن. `@Roles` برای توکن انسانی دست‌نخورده ماند.
- **`docs/26-branch-registry.md`** — رجیستری شاخه‌ها و هماهنگی کار موازی، با
  نیمهٔ الزام‌آورش در `AGENTS.md § ۶`. پس از سه تداخل واقعی در یک روز ساخته شد.
  (PR #49)
- **چهار نقص واقعی** که همه زیر زیرساخت واقعی بیرون آمدند: D-028، D-029،
  D-030 (بخش ۲۲) و تداخل معنایی `@AllowService` که هیچ ابزار Merge نشانش نداد.
...
> **به‌روزرسانی 2026-09-24.** `/assets/[id]/timeline` ادغام شد (PR #80) —
> **هر ده صفحهٔ P0 که `EXP-002` نام برده، اکنون روی `main` هستند.** جداگانه،
> یک یافتهٔ Audit امنیتی P0 هم بسته شد (PR #81، `identity-service`؛ صفحه
> نیست، رفتار سرویس). از `EXP-003` («Economic and marketplace web surfaces»)
> بخش فقط‌خواندنش — `/wallet`، `/marketplace`، `/marketplace/[productId]` —
> روی شاخه‌ای باز ساخته شد؛ `/orders` (نوشتنی) جداگانه در دست نشست دیگری است.
>
> **به‌روزرسانی دوم 2026-09-23.** `/organizations` هم روی `main` نشست
> (PR #79). `/assets/[id]/timeline` — آخرین صفحهٔ P0 نقشهٔ صفحات `EXP-002`
> (`docs/16 § ۱۶٫۶`) — روی این شاخه ساخته شد و منتظر ادغام است. با ادغام آن،
> **هر ده صفحهٔ P0 که `EXP-002` نام برده، روی `main` می‌نشینند.**
>
> **به‌روزرسانی 2026-09-23.** `/drivers` و `/drivers/[id]` روی `main` نشستند
> (فهرست، ثبت، ویرایش، تغییر وضعیت، تخصیص/پایان تخصیص). `ثبت کارکرد`
> (`/usage`، به‌روزرسانی پیشین همین بخش که کهنه بود) هم از پیش روی `main`
> است. از `EXP-002` تنها **سازمان و اعضا** مانده — که هم‌زمان با این کار توسط
> نشست دیگری ساخته می‌شود.
>
> **به‌روزرسانی 2026-09-22.** `/maintenance` و `/maintenance/[id]` روی `main`
> نشستند. آنچه از `EXP-002` مانده: راننده و تخصیص، ثبت کارکرد، و سازمان و
> اعضا — و هیچ صفحه‌ای که **بنویسد** هنوز ساخته نشده.
>
> **به‌روزرسانی دوم 2026-09-21.** پورتال دیگر بدون صفحهٔ دامنه‌ای نیست: ورود،
> داشبورد، `/assets` و `/assets/[id]` روی `main` هستند (PR #66 و #67). آنچه از
> `EXP-002` مانده: راننده و تخصیص، ثبت کارکرد، نگهداری، و سازمان و اعضا — و
> هیچ صفحه‌ای که **بنویسد** هنوز ساخته نشده. بعد از آن `EXP-003` تا `EXP-005`.
> E2E مرورگری دیگر بدهی نیست (بخش ۲۳).
>
...
- Frontend — **بقیهٔ صفحه‌های دامنه‌ای**: `EXP-002` هر ده صفحهٔ P0 خود را
  روی `main` دارد (تمام). از `EXP-003` («Economic and marketplace web
  surfaces»)، `/wallet`، `/marketplace` و `/marketplace/[productId]` روی
  یک شاخهٔ باز ساخته شد؛ `/orders` مانده — نشست دیگری در دست دارد.
  `EXP-004` و `EXP-005` شروع نشده‌اند. `apps/admin` اصلاً وجود ندارد
  (ADR-058 § ۲)
...
> **این جدول عکسی از 2026-08-28 است و عمداً دست‌نخورده می‌ماند.** وضعیت واقعی
...
### به‌روزرسانی 2026-09-24 — `/wallet`، `/marketplace`، `/marketplace/[productId]`: بخش فقط‌خواندنی `EXP-003`

سه صفحه: **کیف پول و تراکنش** (موجودی، وجوه در وثیقه، تاریخچهٔ تراکنش با
پالایه و صفحه‌بندی، به‌علاوهٔ یک فرم افزایش موجودی نمایشی)، **جست‌وجوی کالا و
خدمت** (فهرست با جست‌وجوی متنی، دسته و ترتیب)، و **مقایسهٔ پیشنهادها** (هر
پیشنهاد منتشرشده روی یک کالا، از هر تأمین‌کننده‌ای). سه‌تای اول از `EXP-003`
(«Economic and marketplace web surfaces»)؛ `/orders` (نوشتنی) بیرون از دامنهٔ
این شاخه ماند — نشست دیگری در دست دارد.

> **افزایش موجودی، شبیه‌سازی است و همین‌جا هم گفته می‌شود.** `CLAUDE.md`:
> «هرگز ادعای اتصال بانکی نکن.» فرم فقط یک فیلد دارد — مبلغ — بدون شمارهٔ
> کارت، بدون نام بانک، بدون نشان «پرداخت امن». پیش از فرم، بنر افشای
> `economic-service`'s خودش (`GET /v1/wallets/provider`) نشان داده می‌شود؛
> اگر آن خواندن شکست بخورد، فرم اصلاً نمایش داده نمی‌شود — یک فیلد مبلغ بدون
> افشا بالای آن، شکل یک پرداخت واقعی می‌گرفت.
>
> **یک شکاف واقعی Backend: `GET /v1/products/:id` نبود.** صفحهٔ مقایسه به
> نام و دستهٔ خود کالا نیاز داشت، ولی تنها راه، جست‌وجوی متنی با شناسه به‌جای
> عبارت بود — نه قابل اتکا (تطبیق Trigram روی نام است، نه شناسه). یک
> Endpoint تازه افزوده شد (`catalogue.controller.ts`/`catalogue.service.ts`)،
> با همان قاعدهٔ بازِ Cross-Tenant که `search` دارد (ADR-042 § 3) و همان
> قاعدهٔ `ACTIVE` که `createOffer` برای همین ستون اعمال می‌کند — کالای
> بایگانی‌شده همان ۴۰۴ کالای ناموجود را می‌گیرد، نه یک رفتار تازه. Commit
> جداگانه، با پنج آزمون یکپارچگی‌سازی تازه (از جمله یک آزمون شکل پاسخ که
> نبودِ `organizationId` را تأیید می‌کند) که **زنده در برابر PostgreSQL واقعی
> اجرا و سبز شد** — نه فقط Typecheck.

**شواهد.** پورتال: ۷۴۶ آزمون (بالاتر از ۷۴۳، افزایش از ادغام PR #81)، شامل
`jest-axe` روی هر سه صفحه؛ `turbo run lint typecheck build --filter=@rasta/web`
سبز؛ `next build` هر سه مسیر را `ƒ` می‌سازد. Backend: `turbo run lint
typecheck build --filter=@rasta/marketplace-service` سبز؛ ۴۰ آزمون
یکپارچگی‌سازی `api-catalogue.int-spec.ts` — شامل پنج تای تازه — زنده در برابر
Postgres واقعی (`docker ps` نشان داد زیرساخت بالا بود) اجرا و سبز شدند، نه
فقط نوشته‌شده.

**آنچه ادعا نمی‌شود:** `/orders` هنوز نیست. `EXP-003` همچنان `IN_PROGRESS`
می‌ماند تا آن صفحه هم برسد.

### به‌روزرسانی 2026-09-24 — P0 امنیتی: سندی که `CLEAN` اعلام شد، از راه URL بارگذاری‌اش هنوز قابل بازنویسی بود

یافتهٔ تأییدشدهٔ PM: `createUploadUrl` یک PutObject قابل استفادهٔ مجدد برای
همان کلید امضا می‌کند، با TTL تا ۳۶۰۰ ثانیه. `finalize` مقدار `checksum` را
از ETag همان لحظه می‌نویسد، ولی نه Worker اسکن و نه `createDownloadUrl` بعداً
آن را بررسی می‌کنند — هر دو فقط کلید را می‌خوانند. پس: بارگذاری پاک، Finalize،
انتظار برای `CLEAN`، سپس PUT بایت‌های دیگر روی همان URL. ردیف همچنان `CLEAN`
می‌ماند و بایت‌های تازه را سرو می‌کند.

**راه‌حل (ترجیح PM، همان‌طور که خواسته شد توجیه می‌شود):** `finalize` — بلافاصله
پس از خواندن Metadata و پیش از ثبت هر ردیفی — بایت‌های همان لحظه را با یک
Copy سمت-سرور به کلیدی زیر پیشوند تازه (`documents-sealed/...`) می‌بندد که
هیچ اعتبارنامهٔ بارگذاری هرگز برایش امضا نشده و نمی‌شود؛ Worker اسکن و
`createDownloadUrl` هر دو از این پس فقط همین کلید را می‌خوانند. جایگزین‌های
دیگر (VersionId ثابت‌شده، بررسی ETag/IfMatch جدا در اسکن و دانلود) رد شدند
چون هرکدام باید در دو جای مستقل درست پیاده شود تا کار کند؛ Copy یک‌باره در
یک نقطه، با `CopySourceIfMatch` بسته به ETag همان `head()`، هر دو سر بازهٔ
مسابقه را می‌بندد: هم بازنویسی بعد از سیل، هم بازنویسی بین خواندن و سیل خودِ
Finalize. کلید خام پس از سیل با `storage.remove()` تلاش‌محور پاک می‌شود.

> **شکاف دوم که همان بازبینی PM اسمش را برد: اندازهٔ PUT بسته‌بندی‌نشده بود.**
> `createUploadUrl` مثل `content-type` حالا `content-length` را هم امضا
> می‌کند. کلاینت HTTP این هدر را از بدنهٔ واقعی خودش می‌سازد و نمی‌تواند دروغ
> بگوید، پس بدنه‌ای با اندازهٔ متفاوت همان‌جا در Storage با عدم‌تطبیق امضا رد
> می‌شود — همان قاعده‌ای که `content-type` از پیش داشت، فقط گسترش‌یافته به
> اندازه.

**شواهد — زنده، نه فقط Typecheck.** ۲۸۳ آزمون واحد و ۱۸۵ آزمون یکپارچگی‌سازی
(`api-document`، `clamav-scan`، `document-lifecycle`، `storage-and-outbox`،
`b3-stream-sequence`، `outbox-durable-claim`) در برابر PostgreSQL، MinIO و
ClamAV واقعی — همه سبز، از جمله دو آزمون امنیتی هستهٔ این کار
(`document-lifecycle.int-spec.ts`, «after finalize seals the object»): یکی
بازنویسی را **بعد از** `CLEAN` شدن با همان URL بارگذاری امتحان می‌کند و ثابت
می‌کند سرو نمی‌شود، دیگری بازنویسی را **قبل از** تیک Worker امتحان می‌کند و
ثابت می‌کند اسکن‌شده هم عوض نمی‌شود — هر دو با بایت‌هایی هم‌طول و هم‌نوع با
اصل، تا فقط قفل Sealing امتحان شود، نه بستن اندازه/نوع که به‌تنهایی کافی
نبود اثبات کند. `turbo run lint typecheck build --filter=@rasta/document-service`
سبز.

**آنچه ادعا نمی‌شود:** کلید خام تا لحظهٔ پاک‌سازی همچنان قابل PUT است — این
دیگر مسئله‌ای نیست چون هیچ‌کس دیگر آن را نمی‌خواند، ولی پاک‌سازی خودش
تلاش‌محور است و شکستش فقط یک شیء یتیم به‌جا می‌گذارد، نه یک ردیف قابل‌سرو.

---

### به‌روزرسانی دوم 2026-09-23 — `/assets/[id]/timeline`: آخرین صفحهٔ P0 نقشهٔ `EXP-002`

`/assets/[id]/timeline` تاریخچهٔ کامل یک دارایی را نشان می‌دهد — همان
`TimelineEntryView` که `/assets/[id]`'s دوسیه از آن فقط ده مورد اخیر را نشان
می‌دهد، اینجا با پالایهٔ بخش و صفحه‌بندی نشانه‌ای، تازه‌ترین در بالا. فقط‌خواندنی
است؛ هیچ فرمی ندارد. از دوسیهٔ دارایی (بخش «رویدادهای اخیر») به آن پیوند داده
شد؛ در `nav.ts` نمی‌نشیند چون هیچ‌کجای `docs/16` آن را در ناوبری اصلی نگذاشته.

> **جدول برچسب بخش‌های تاریخچه، با واقعیت Backend نمی‌خواند.** `labels.ts`
> از قبل `timelineCategoryLabel` داشت، ولی نُه بخش نادرستی نگاشت می‌کرد
> (`REGISTRATION`، `ASSIGNMENT`، `COMPLIANCE`، `LOCATION`، `STATUS`) که هیچ‌کدام
> در `TIMELINE_CATEGORIES` واقعی سرویس دارایی (`LIFECYCLE`، `USAGE`،
> `MAINTENANCE`، `INSURANCE`، `INSPECTION`، `DOCUMENT`، `COST`، `PROJECT`،
> `TRANSFER`) نیستند — نتیجه‌اش پنج بخش واقعی که تا امروز روی صفحهٔ دوسیه
> ترجمه‌نشده می‌ماندند و بدون آنکه کسی متوجه شود. جدول با شمارهٔ واقعی جایگزین
> شد، همان یک نگاشت، برای دوسیه و تاریخچهٔ کامل هر دو.

**شواهد.** ۶۷۵ آزمون پورتال (یک فایل Spec تازه با `jest-axe`، به‌علاوهٔ اصلاح
جدول برچسب که آزمون تازه‌ای نخواست چون فقط یک مصرف‌کنندهٔ موجود داشت)، `turbo
run lint typecheck build --filter=@rasta/web` سبز، `next build` مسیر تازه را
`ƒ` می‌سازد. **زنده در برابر Stack واقعی اجرا نشد** — این‌بار حتی `docker
compose` بالا نبود (نه فقط زیرساخت کامل)، پس تأیید همین‌جا صادقانه `Unit +
Axe + Build` است، نه زنده.

**آنچه ادعا نمی‌شود:** با ادغام این PR، هر صفحه‌ای که `EXP-002`
(`planning/backlog.json`: «Identity, organization, asset, fleet and
maintenance web surfaces») نام برده روی `main` می‌نشیند — ولی خودِ `EXP-002`
هنوز `IN_PROGRESS` می‌ماند، چون معیار پذیرشِ دومش («Desktop و Viewport های
موبایل پشتیبانی‌شده، آزمون دسترس‌پذیری و جداسازی مستأجر را پاس کنند») هنوز
Evidence ندارد. `/rewards` و `/rewards/history` به این Epic تعلق ندارند —
عنوانشان در `docs/16` هست، ولی به‌گواهی `planning/backlog.json` بخشی از
`EXP-003` («Economic and marketplace web surfaces») هستند.

---

### به‌روزرسانی 2026-09-23 — راننده و تخصیص: نخستین صفحه‌های **نویسنده** این دور

`/drivers` (فهرست + ثبت راننده) و `/drivers/[id]` (پرونده + ویرایش + تغییر
وضعیت + تخصیص/پایان تخصیص) روی همان زیرساخت نوشتن ساخته شدند که PR #75
(`/usage`) بنا گذاشت: `csrf.ts`، `submission.ts` (کلید یکتایی درخواست که در
تلاش دوباره هم همان می‌ماند)، و `write.ts`. تخصیص، مسیر جدا ندارد — از پروندهٔ
راننده باز و بسته می‌شود، چون `docs/16` صفحه‌ای برایش نام نبرده بود.

> **`writeThroughGateway` فقط `POST` می‌فرستاد.** ویرایش راننده و تغییر
> وضعیت هر دو `PATCH` می‌خواهند؛ `WriteCall` یک `method?` اختیاری گرفت که
> نبودنش هنوز `POST` می‌شود — یک شکاف واقعی در زیرساخت PR #75، نه ابزار تازه.
>
> **شناسهٔ رکورد، میدان فرم نیست.** `useActionState` فقط `(previous,
formData)` می‌دهد؛ راه ساده افزودن `driverId`/`assignmentId` به‌صورت میدان
> پنهان بود — ولی این زمینهٔ مسیر را که خود صفحه از پیش می‌داند با محتوای
> فرم که کاربر می‌تواند دستکاری کند قاطی می‌کرد. با `.bind(null, driverId)`
> جایگزین شد؛ این دو شناسه دیگر در Zod اعتبارسنجی نمی‌شوند، پارامتر تابع‌اند.
>
> **یک اشکال واقعی، با تست پیدا شد نه با مرور.** `ChangeStatusForm` فقط
> وضعیت فعلی را از گزینه‌ها کنار می‌گذاشت، نه انتقال‌های غیرمجاز را — یعنی
> راننده‌ای با وضعیت پایانی `DEACTIVATED` (که در `driver-lifecycle.ts` هیچ
> انتقال مجازی ندارد) همچنان `ACTIVE` و `SUSPENDED` را در فرم می‌دید. رفع شد
> با آینهٔ سمت-کلاینت همان جدول `TRANSITIONS`؛ این فیلتر فقط UI است — همان
> قاعدهٔ `docs/16 § ۱۶٫۱۱` که «پنهان‌کردن کنترل، خودش کنترل امنیتی نیست» و
> اجرای واقعی هنوز روی fleet-service می‌ماند.

**شواهد.** ۵۸۹ آزمون پورتال (۱۱ فایل Spec تازهٔ این دو صفحه/دو ماژول سرور،
شامل `jest-axe` روی هر پنج فرم)، `turbo run lint typecheck build
--filter=@rasta/web` سبز، `next build` هر دو مسیر را `ƒ` می‌سازد. **زنده در
برابر Stack واقعی اجرا نشد** — همان استدلال هزینه/ارزش صفحه‌های نگهداری؛
تأیید همین‌جا صادقانه `Unit + Axe + Build` است، نه زنده.

**آنچه ادعا نمی‌شود:** `/organizations` هنوز نیست (هم‌زمان، نشست دیگری
می‌سازدش). `EXP-002` همچنان `IN_PROGRESS`.

---

### به‌روزرسانی 2026-09-22 — دومین جفت صفحهٔ دامنه‌ای: `/maintenance` و `/maintenance/[id]`

`/maintenance` و `/maintenance/[id]` روی همان الگوی `/assets`/`/assets/[id]`
(PR #67) ساخته شدند: فهرست درخواست‌های نگهداری با پالایه (وضعیت، نوع، وخامت)
و صفحه‌بندی نشانه‌ای، و **جزئیات یک درخواست** — شناسنامه، روند کار، هزینه و
ارجاع‌های تعمیرگاه. هر دو روی سرور و از راه Gateway داده می‌گیرند؛ هیچ فرم و
هیچ نوشتنی ندارند — زیرساخت آن (CSRF، کلید یکتایی درخواست) هنوز در پورتال
نیست و دور دیگری از `EXP-002` است، موازی با این کار، سمت دیگری می‌سازدش.

> **روند کار، محاسبه‌شده است نه ذخیره‌شده.** `MaintenanceRequestDetail` یک
> Timestamp به‌ازای هر نقطهٔ عطف دارد (`startedAt`، `completedAt`،
> `approvedAt`، `cancelledAt`…) که تا رخ‌ندادن، `null` است. نمایش بی‌قید هر
> فیلد چهار ردیف خالی برای درخواستی یک‌ساعته نشان می‌داد؛ این صفحه فقط
> نقطه‌های واقعاً رخ‌داده را به یک فهرست ترتیبی تبدیل می‌کند — همان ایدهٔ
> «هر مانع اعزام، نه فقط اولی» در پروندهٔ دارایی.
>
> **جزئیات درخواست، ارجاع تعمیرگاه را از همان پاسخ می‌گیرد.** `GET
/v1/maintenance-requests/{id}` خودش آرایهٔ `repairOrders` را با وضعیت و
> هزینه هر سفارش برمی‌گرداند؛ فراخوانی دوم به `GET /v1/repair-orders/{id}`
> لازم نبود — آن Endpoint فقط برای Parts/Labour/Cost خط‌به‌خط لازم می‌شود که
> این صفحه نشانش نمی‌دهد.
>
> **`OPEN` در واژگان بصری کم بود.** `StatusBadge` پنج وضعیت درخواست نگهداری
> را از پیش می‌شناخت (`IN_PROGRESS`، `COMPLETED`، `APPROVED`، `CANCELLED`) —
> فقط `OPEN` جا افتاده بود و به‌عنوان `warning` افزوده شد؛ دقیقاً همان الگوی
> افزودن سه وضعیت دارایی در PR #67.

**شواهد.** ۴۰۳ آزمون پورتال (۱۶ آزمون تازهٔ این دو صفحه، شامل `jest-axe` روی
هر دو)، `turbo run lint typecheck build --filter=@rasta/web` سبز، و
`next build` تولیدی هر دو مسیر را به‌صورت `ƒ` (Server-Rendered on Demand)
می‌سازد — همان چیزی که Job `Portal in a browser` در CI در برابرش اجرا
می‌شود. **زنده در برابر Stack واقعی اجرا نشد** — Keycloak و زیرساخت بالا
بودند، ولی بالا آوردن هر پنج سرویس Backend به‌همراه Migration و Seed برای
یک تغییر فقط‌خواندنی، هزینه‌اش از ارزشش بیشتر بود؛ تأیید همین‌جا صادقانه
`Unit + Axe + Build` است، نه زنده.

**آنچه ادعا نمی‌شود:** `/organizations`، `/drivers` و `/usage` هنوز نیستند.
`EXP-002` همچنان `IN_PROGRESS` و صفر امتیاز است.

---

### به‌روزرسانی 2026-09-21 (سوم) — نخستین صفحه‌های عملیاتی

`/assets` و `/assets/[id]` روی `main` نشستند: فهرست ماشین‌آلات با پالایه و
صفحه‌بندی، و **پروندهٔ الکترونیکی دارایی** — شناسنامه، آمادگی بهره‌برداری،
هزینه‌ها و رویدادهای اخیر. هر دو روی سرور و از راه Gateway داده می‌گیرند.

> **دو تصمیم که ارزش نگه‌داشتن دارند.** نخست: پالایه و صفحهٔ بعد هر دو در URL
> زندگی می‌کنند، نه در حافظهٔ مرورگر. یعنی یک فهرست پالایش‌شده لینکی است که
> می‌شود برای همکار فرستاد، و کل صفحه بدون یک خط جاوااسکریپت کار می‌کند. دوم:
> صفحهٔ پرونده **همهٔ** موانع اعزام را نشان می‌دهد، نه اولی را — چون
> `asset-service` هم همه را برمی‌گرداند، و اپراتوری که بیمه را تمدید کند و
> برگردد تا معاینهٔ منقضی را ببیند، دو بار رفته برای چیزی که یک‌بار گفتنی بود.
>
> **سه وضعیت به زبان بصری افزوده شد** (`docs/16 § ۱۶٫۵`): `ASSIGNED`،
> `REGISTERED` و `DECOMMISSIONED` وضعیت‌های واقعی `asset-service` بودند و جدول
> نامشان نبرده بود، پس هر سه خاکستری می‌افتادند — که برای «تخصیص‌یافته» و
> «از رده خارج» غلط بود.

**شواهد.** ۳۷۸ آزمون پورتال، سه آزمون مرورگری، و دروازهٔ کامل `pnpm verify` با
کد خروجی صفر.

**آنچه ادعا نمی‌شود:** `/organizations`، `/drivers`، `/usage` و صفحه‌های
نگهداری هنوز نیستند، و در ناوبری هم نمی‌نشینند — پیوند به صفحه‌ای که پاسخ
نمی‌دهد یک قول است که محصول به آن عمل نمی‌کند. `EXP-002` همچنان
`IN_PROGRESS` و صفر امتیاز است.

---

### به‌روزرسانی 2026-09-21 (دوم) — پورتال نشست دارد و نخستین صفحهٔ واقعی‌اش را

**کسی می‌تواند وارد شود.** `EXP-002` شروع شد: جریان `Authorization Code + PKCE`
در Route Handler های Next.js، Cookie نشست مهرشده با `AES-256-GCM` که
`HttpOnly` و `Secure` و `SameSite=Strict` است، کلاینت سمت سرور که **فقط** به
API Gateway اجازهٔ تماس دارد، صفحهٔ `/login`، و داشبوردی که نام و سازمان فعال و
نقش‌های مؤثر کاربر را از سرویس هویت — از راه Gateway، روی سرور — می‌آورد.

> **این کار یک تناقض در `docs/16` پیدا کرد و با `ADR-059` بست، نه با حدس.**
> `§ ۱۶٫۱` احراز هویت را به `oidc-client-ts` می‌سپرد — کتابخانه‌ای که در
> مرورگر اجرا می‌شود — و `§ ۱۶٫۱۱` می‌خواست Refresh Token در Cookie با
> `HttpOnly` بنشیند. یک جاوااسکریپت مرورگر **نمی‌تواند** چنین Cookie ای
> بنویسد؛ معنای `HttpOnly` همین است. قاعده ماند و ابزار عوض شد: جریان به سرور
> رفت و هیچ توکنی به جاوااسکریپت مرورگر نمی‌رسد.

> **نخستین Job مرورگری این مخزن هم همین‌جا آمد.** `apps/web/playwright.config.ts`
> از `EXP-001` نوشته بود که «یک Job مرورگری که روی هیچ صفحه‌ای اجرا شود هیچ
> ادعایی نمی‌کند» و با نخستین سطح واقعی وارد خط لوله می‌شود. سه چیز را می‌سنجد
> که فقط مرورگر می‌تواند نشان دهد: سند واقعاً راست‌به‌چپ و فارسی سرو می‌شود،
> راهِ ورود بدون جاوااسکریپت کار می‌کند، و **`localStorage` و `sessionStorage`
> هر دو خالی‌اند** — که ادعای مرکزی `ADR-059` است.

**شواهد.** ۳۵۴ آزمون پورتال (۶۷ تای آن سمت سرور)، سه آزمون مرورگری در برابر
Build تولیدی، و دروازهٔ کامل `pnpm verify` با کد خروجی صفر و حذف کامل متغیرهای
پایگاه داده.

**آنچه ادعا نمی‌شود:** `/organizations`، `/assets`، `/assets/[id]`، `/drivers`،
`/usage`، `/maintenance` و `/maintenance/[id]` هنوز ساخته نشده‌اند. `EXP-002`
تمام نیست و صفر امتیاز دارد.
...
**روی `main` نشست** با PR #60 (`e6d509e`). آنچه ساخته شد:
جدول `notification_preference` با Migration برگشت‌پذیر، نردبان تقدم پنج‌لایه،
سه Endpoint (`GET`/`PUT /v1/preferences` و `GET /v1/preferences/effective` که
**لایهٔ برنده** را برمی‌گرداند)، و سیم‌کشی به مسیر تحویل: ترجیحِ خاموش یک
`notification_delivery` با وضعیت `SUPPRESSED` و دلیل بوجود می‌آورد، نه یک ردیف
غایب.

**شواهد.** محلی (Windows، Postgres و Kafka واقعی): ۲۰۸ واحد + ۱۱۰ یکپارچگی سبز،
و `verify-migration-reversible.mjs notification` سبز — up → down → up روی Schema
یک‌بارمصرف، با هر چهار Migration این سرویس. در CI: اجرای `35476698542` روی
Merge Ref، با هر دو Suite ترجیحات سبز.

> **نخستین اجرای CI این شاخه قرمز شد، و درسش را نگه دارید: هر ۱۱۰ تست سبز
> بودند.** اجرای `35474555558` با `Jest did not exit one second after the test
run has completed` و `ReferenceError: You are trying to import a file after
the Jest environment has been torn down` از `audit-events.int-spec.ts` مرد —
> بیست دقیقه بعد از پایان تست‌ها. **Hang بود، نه Failure**، و ریشه‌اش یک نقص
> واقعی روی `main` در `src/outbox/kafka.publisher.ts` بود: `producer` فقط پس از
> موفقیت `connect()` ثبت می‌شد، پس خاموشیِ وسط یک connect چیزی برای بستن نداشت و
> Timerهای Retry کافکا حلقهٔ رویداد را باز نگه می‌داشتند؛ و هیچ‌چیز جلوی
> `getProducer()` را نمی‌گرفت که **پس از** نابودی ماژول یک Producer تازه باز کند
> (`OutboxRelay.stop()` فقط یک مهلت کراندار صبر می‌کند). هر دو نیمه بسته شد و
> `kafka.publisher.spec.ts` پوشش‌شان می‌دهد — که روی کد قدیمی **می‌افتد**، و
> مورد دومش ۱۲۰ ثانیهٔ کامل Hang می‌کند. این نقص از PR #58 روی `main` بود و
> هیچ‌کس ندیده بودش چون فقط وقتی می‌گزد که Broker وسط اجرا برود.

> **سه انحراف، همه در `ADR-054-implementation-plan § ۴` ثبت شده‌اند.**
> مهم‌ترینش: **ساعات سکوت ساخته نشد** (معیار پذیرش سوم NTF-003 بی‌شاهد است).
> دلیلش این است که ساعات سکوت خاصیتِ کانالی است که کسی را **قطع** می‌کند؛
> `IN_APP` این کار را نمی‌کند و تعویقش فقط شمارندهٔ نخوانده را غلط می‌کند. با
> `NTF-004` می‌آید. دو انحراف دیگر: `422` فقط تست واحد دارد (هیچ قاعده‌ای امروز
> `MANDATORY` نیست، پس از HTTP قابل‌رسیدن نیست)، و سناریوی E2E ۲ اجرا نشد چون
> Project `notification-api` در Playwright هنوز وجود ندارد — همان کمبود E2E ۱.

**گام بعدی، به ترتیب:** (۱) `Q-37` و `NTF-004` — تنها مسدودکنندهٔ باقی‌ماندهٔ
`COM-008`، که ساعات سکوت را هم با خودش می‌آورد؛ (۲) `Q-56` و زیرگام کیفیت
`COM-005`؛ (۳) `EXP-002`.

---

### به‌روزرسانی 2026-09-19 — هر سه کار § ۲۹ انجام شد، و دو شکاف حسابرسی بسته شد

هر سه کاری که نسخهٔ ۰۹-۱۸ پایین‌تر فهرست کرده بود انجام شد، و بعدش دو مورد
دیگر. `main` روی `419afde` سبز است. پنج PR:

| PR                                                | چه چیزی                                                         |
| ------------------------------------------------- | --------------------------------------------------------------- |
| [#54](https://github.com/marabi766/RASTA/pull/54) | `EXP-001` الف — Workspace `apps/web` و زنجیرهٔ کیفیت، `ADR-058` |
| [#55](https://github.com/marabi766/RASTA/pull/55) | `EXP-001` ب — Token ها، فونت محلی، لایهٔ ارائهٔ فارسی           |
| [#56](https://github.com/marabi766/RASTA/pull/56) | `EXP-001` پ — کتابخانهٔ Component و سه حالت اجباری              |
| [#57](https://github.com/marabi766/RASTA/pull/57) | `D-011` — `MAINTENANCE_SCHEDULE_CHANGED`                        |
| [#58](https://github.com/marabi766/RASTA/pull/58) | انحراف `ADR-054 § ۳` — Outbox و سه رویداد `NOTIFICATION_*`      |

(کار ۱ و ۲ — `backlog.json` و شاخهٔ `docs/audit-immutable-archive` — شب پیش
در #52 و #53 فرود آمدند.)

**پوشهٔ `apps/` دیگر خالی نیست.** دوازده سرویس Backend با CI سبز داشتیم که هیچ
انسانی نمی‌توانست از آن‌ها استفاده کند؛ `EP-EXPERIENCE` روی ۰٪ از ۸۹ امتیاز
ایستاده بود و همین `RUN-004` را `BLOCKED` نگه داشته بود. حالا یک پورتال فارسی
راست‌به‌چپ هست با ۲۸۳ تست، ۳۰ Snapshot دوجهته، و تضاد رنگ **اندازه‌گیری‌شده** در
هر دو تم. `EXP-001` پذیرفته شد و MVP از **۳۶٫۸٪ به ۳۹٫۱٪** رفت.

**دو شکاف حسابرسی که ماه‌ها باز بودند بسته شدند، و هر دو یک جنس داشتند:**
تصمیمی که در یک سرویس رخ می‌داد و `audit-service` — که تنها ورودی‌اش رویداد
است — هرگز نمی‌دیدش.

- `D-011`: تغییر برنامهٔ سرویس هیچ رویدادی تولید نمی‌کرد؛ دلیلش فقط به ستون
  `notes` الحاق می‌شد. از ۶ شهریور باز بود و از ۱۹ شهریور که `audit-service`
  آمد، «واقعی» شده بود.
- `ADR-054 § ۳`: خواندن و نادیده‌گرفتن اعلان رویداد نداشت. ADR خودش نوشته بود
  «این یک انحراف از قاعدهٔ الزام‌آور است، نه یک انتخاب دامنه» و دو راه گذاشته
  بود: انتشار رویداد، یا پذیرش مکتوب استثنا. **راه دوم رد شد** — `S-06` یک
  قاعدهٔ امنیتی است و استثنای مستند روی آن، سطح حسابرسی پلتفرم را دائماً پایین
  می‌آورد نه یک‌بار.

در هر دو، رویداد در **همان تراکنشی** نوشته می‌شود که وضعیت را عوض می‌کند.
در دومی نکتهٔ ظریف‌تری هم هست: رویداد فقط وقتی منتشر می‌شود که ردیفی **واقعاً**
جابه‌جا شده باشد. به‌روزرسانی شرطی همان چیزی است که این Endpoint ها را
Idempotent می‌کند، و شمارندهٔ سطرهایش تنها پاسخ صادقانه به «آیا این فراخوان
چیزی را عوض کرد» است؛ انتشار از لایهٔ سرویس دو درخواست هم‌زمان را دو رکورد
حسابرسی می‌کرد.

**`audit-service` تقریباً هیچ تغییری لازم نداشت** — فقط یک ردیف توپولوژی.
`DomainProjectorConsumer` عمداً کامل است و نام رویداد ناشناخته را هم ثبت
می‌کند، چون «انبار حسابرسی که رویداد ناآشنا را بی‌صدا رد کند، دقیقاً همان‌جا
بی‌اعتماد است که بیشترین اهمیت را دارد».

#### چهار چیزی که CI گرفت و محلی دیده نمی‌شد

هر چهار الگوی تکرارشونده‌اند، پس ارزش ثبت دارند:

۱. **Gitleaks روی داده آزمون.** شناسهٔ همبستگیِ واقع‌نما در یک تست، کنار نامی
که شبیه شناسه است، دقیقاً الگوی یک کلید نشت‌کرده دارد. اسکنر دربارهٔ **شکل**
درست می‌گفت؛ داده آزمون عوض شد، نه اسکنر.

۲. **حذف در کامیت بعدی، یافتهٔ Gitleaks را پاک نمی‌کند** — روی یک PR همهٔ
کامیت‌ها دیده می‌شوند. درمانش بازنویسی تاریخچهٔ همان شاخه است.

۳. **نقل‌کردن همان رشته در توضیحِ اینکه چرا بد است، آن را برمی‌گرداند.**
اسکنر یک کامنت یا سند Markdown را همان‌طور می‌خواند که کد را.

۴. **نگهبان کشف Outbox.** دادن Outbox به یک سرویس، آن را جلوی دروازه‌ای
می‌گذارد که تا حالا لازم نبوده به آن فکر کند:
`verify-outbox-claim-migration.mjs` رد کرد که «این سرویس‌ها Outbox دارند و
هیچ‌چیز بررسی‌شان نمی‌کند». سرنخش در یک تست موجود بود («Discovery Guard این
سرویس را درست نادیده می‌گیرد») و دیده شد ولی دنبال نشد.

هر چهار در `docs/26 § ۲۶٫۶` نوشته شدند.

#### گام بعدی

۱. **`Q-37` — Port ایمیل و Adapter ها.** تنها مسدودکنندهٔ باقی‌ماندهٔ
`COM-008`: معیار دوم پذیرش («تحویل ایمیل ترجیحات را رعایت و نتیجه را ثبت
می‌کند») هیچ پیاده‌سازی ندارد — در `src/` هیچ ماژول Channel یا Mail یا Email
نیست. تصمیم ثبت‌شده: Port کانال + Adapter توسعه‌ای Mailpit + Adapter عمومی
SMTP که تماماً از محیط پیکربندی می‌شود. **هیچ فروشنده‌ای در کد** (`AGENTS.md`
§ ۹).

۲. **`Q-56` — سیگنال کیفیت تأمین‌کننده.** `COM-005` روی `IN_PROGRESS` است چون
زیرگام کیفیت (وزن ۳۰٪) عمداً ساخته نشد. تصمیم ثبت‌شده: `rating` موجود در
`REVIEW_SUBMITTED` به‌عنوان سیگنال، با وزن پیکربندی‌پذیر؛ افزایشی و برگشت‌پذیر.

۳. **`EXP-002` به بعد** — صفحه‌های دامنه‌ای، روی پایه‌ای که حالا وجود دارد.

---

### به‌روزرسانی 2026-09-18 — گام بعدیِ توصیه‌شده

برنامهٔ 2026-09-07 پایین‌تر **انجام شد**: `COM-009` و `COM-008` هر دو روی
`main`اند. سه چیز جایش نشسته، به ترتیب اولویت.

**۱. `planning/backlog.json` هفده روز کهنه است** (`asOf: 2026-09-01`). در آن،
`COM-008` و `COM-009` هنوز `READY` یعنی «شروع‌نشده»اند، پس درصد رسمی —
**۳۴٫۴٪ برای MVP** — واقعیت را کمتر از آنچه هست نشان می‌دهد. طبق قاعدهٔ
حکمرانی، فقط واحد **پذیرفته‌شده** امتیاز می‌گیرد، نه واحدی که کدش سبز است.

> **مسدودکنندهٔ پذیرش که فقط صاحب محصول می‌بندد:** `NTF-002` برای خواندن و
> نادیده‌گرفتن اعلان **هیچ رویداد دامنه‌ای منتشر نمی‌کند**، پس `audit-service`
> این دو کنش را هرگز نمی‌بیند. تا این تصمیم گرفته نشود `COM-008` پذیرفته
> نمی‌شود، هرچند کدش کامل و CI‌اش سبز است.

**۲. شاخهٔ معیوب `docs/audit-immutable-archive`** — `ADR-055` تکراری (باید
`ADR-057` شود) و یک `Q-44` سوم. کار کوچکی است و هرچه بماند گران‌تر می‌شود،
چون هر دو شماره روی `main` تثبیت شده‌اند.

**۳. `EXP-001` — پایهٔ وب تولیدی و Design System.**

پوشهٔ `apps/` **اصلاً وجود ندارد**. دوازده سرویس Backend با CI سبز داریم که
**هیچ انسانی نمی‌تواند از آن‌ها استفاده کند**. `EP-EXPERIENCE` روی ۰٪ از ۸۹
امتیاز ایستاده و همین `RUN-004` (خلبانی کنترل‌شده و تصمیم go/no-go) را
`BLOCKED` نگه داشته. ساختن `COM-007` یا `CON-001` پیش از آن، فقط سرویس
سیزدهمی می‌سازد که کسی نمی‌بیند.

این پروژه RTL و فارسی است با تقویم هجری شمسی و اعداد فارسی فقط در لایهٔ ارائه
(`docs/16-ui-architecture.md`) — یعنی `EXP-001` کار پایه‌ای واقعی دارد که
بعداً نمی‌شود چسباند.
...
اجرای سبز `35476698542`. پیش‌تر: 2026-09-18 — شش Merge، `main` روی `f7252d3`،
اجرای سبز `35364991343`؛ و 2026-08-30 (Baseline سنجش پیشرفت).
