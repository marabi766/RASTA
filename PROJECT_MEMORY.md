...
| Audit Trail ماندگار                                | **PARTIAL** — AUD-001 (Projector ده Topic) + AUD-002 (API خواندن مستأجر-امن) + نیمهٔ شواهد دست‌نخوردگی AUD-003 (زنجیرهٔ Hash، `audit_chain_head`، `GET /v1/audit-events/verify` — اثبات‌شده روی PostgreSQL، بدون امضا)؛ رکورد جبرانی (نیمهٔ دوم AUD-003)، صادرات و مسیر B (AUD-004) هنوز نیستند |
...
**به‌روزرسانی 2026-09-12 — ۵۵ ADR موجود (`ADR-001` تا `ADR-055`).** شمارش پایین‌تر در این بخش کهنه است و به فاز اقتصادی
تعلق دارد؛ فهرست معتبر همیشه `docs/21-adr-list.md` است.

سه ADR تازه، هر سه **`Proposed`**؛ ADR-055 و ADR-054 پیاده نشده‌اند و وضعیت اجرای ADR-053 در خود سند ثبت شده است:
...
- **ADR-055 — لنگرگذاری خارجی و بایگانی تغییرناپذیر Audit (Proposed، سخت‌سازی پس از MVP).** Checkpointهای دسته‌ای زنجیرهٔ
  ...
  مدل تفکیک اختیار در Q-44 بازند. این تصمیم مقاومت و اثبات مستقل را بالا می‌برد، نه اینکه امنیت مطلق یا Tamper-proof بودن را
  ادعا کند.
