...
2. همان آزمون واحد **چهار بند تعریف‌کننده پنجره** را هم تثبیت می‌کند: فیلتر
   منتشرنشده، ترتیب قدیمی‌ترین‌اول، کران، و `FOR UPDATE SKIP LOCKED` — با
   `limit` به‌صورت پارامتر و **بدون** فیلتر سازمان.
...
نسخه اول همین را ادعا می‌کرد و به‌محض وجود یک Backlog باستانی بیگانه افتاد —
یعنی همان نقص، یک سطح بالاتر. اکنون فقط ادعا می‌کند ردیف‌های **خودش** داخل
پنجره، قدیمی‌ترین‌های خودش‌اند و پیوسته: ردیف n تنها وقتی داخل است که هر ردیف
قدیمی‌تر از آن هم داخل باشد.
