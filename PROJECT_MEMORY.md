...
>
> **Baseline رسمی پیشرفت:** `planning/backlog.json` اکنون ۴۷ آیتم در ۱۲ Epic را
> نگه می‌دارد. فقط `ACCEPTED` همراه Evidence امتیاز می‌گیرد؛ MVP و محصول کامل
> جدا محاسبه می‌شوند و CI گزارش stale یا Backlog نامعتبر را رد می‌کند. Baseline
> اولیه تا تأیید مالک محصول عمداً `PROVISIONAL` است (`docs/25`).
>
> ---
>
...
| D-020 | **`z.coerce.boolean()` هر رشته ناتهی — از جمله `"false"` — را `true` می‌خواند**  | **بالا**                        | پنج Flag محیطی پلتفرم قابل خاموش کردن نیستند؛ اپراتور `false` می‌نویسد و رفتار عوض نمی‌شود       | Parser درست (`booleanEnv`) در `@rasta/config` هست؛ پنج Flag دیگر یک Task مستقل‌اند — پایین‌تر               |
...
`"no"` و `"off"`. یعنی این Flagها را نمی‌توان خاموش کرد. اپراتور مقدار را
می‌گذارد، پیکربندی خودش را می‌خواند، و باور می‌کند قابلیت غیرفعال است. یک Flag
که خاموش نمی‌شود، از نبودن Flag بدتر است.

**Flagهای دقیقاً متأثر — هر پنج مورد:**

| متغیر                          | فایل                                             | پیش‌فرض | اثر «خاموش نشدن»                                                                     |
| ------------------------------ | ------------------------------------------------ | ------- | ------------------------------------------------------------------------------------ |
| `OTEL_TRACES_ENABLED`          | `packages/config/src/env.ts` (`baseEnvSchema`)   | `true`  | Tracing در هر سرویس روشن می‌ماند                                                     |
| `KAFKA_SCHEMA_STRICT`          | `packages/config/src/env.ts` (`kafkaEnvSchema`)  | `true`  | اعتبارسنجی سخت‌گیرانه Schema روشن می‌ماند                                            |
| `GATEWAY_RATE_LIMIT_FAIL_OPEN` | `services/api-gateway/src/config/env.ts`         | `true`  | **امنیتی:** با قطع Redis ترافیک عبور می‌کند، حتی اگر اپراتور Fail-Closed خواسته باشد |
| `KEYCLOAK_SYNC_ENABLED`        | `services/identity-service/src/config/env.ts`    | `true`  | همگام‌سازی Keycloak روشن می‌ماند                                                     |
| `MARKETPLACE_TEMPORAL_ENABLED` | `services/marketplace-service/src/config/env.ts` | `true`  | سرویس در محیط بدون Temporal تلاش به اتصال می‌کند                                     |

**در این PR عمداً اصلاح نشدند.** هر کدام در لحظه‌ای که درست Parse شود، رفتار یک
سرویس در حال اجرا را عوض می‌کند: اپراتوری که `OTEL_TRACES_ENABLED=false`
نوشته، تا امروز با Tracing روشن کار کرده و اصلاح Parser آن را خاموش می‌کند.
این یک تغییر مؤثر بر استقرار است، نه یک پاکسازی — و به‌عنوان اثر جانبی یک PR
مربوط به `document-service` وارد شدنش، همان شکلی است که یک تغییر پیکربندی
بی‌صدا به Production می‌رسد.

**Task مستقل بعدی — پس از Merge شدن PR سند:**

1. **همه** متغیرهای محیطی Boolean را در کل Repository فهرست کن (نه فقط این پنج
   مورد؛ پارامترهای Query هم همین نقص را دارند).
2. یک Parser مشترک و **تست‌شده** معرفی کن — `booleanEnv` در
   `@rasta/config` امروز همین است و آزمون دارد؛ کار، یکسان‌سازی است نه اختراع.
3. **پیش از استقرار، تغییر رفتار زمان اجرای هر Flag را جداگانه ارزیابی کن** و
   بنویس کدام محیط امروز مقداری دارد که فردا معنایش عوض می‌شود.

**آنچه در این PR اصلاح شد:** فقط `document-service` — پارامتر Query
`includeDeleted` که `?includeDeleted=false` را `true` می‌خواند و دقیقاً همان
ردیف‌هایی را برمی‌گرداند که فراخوان خواسته بود کنار بگذارد. `booleanEnv` جای
Coercion نشست و آزمون گرفت.
