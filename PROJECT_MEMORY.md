...
| D-027 | ترتیب معنایی رویدادهای یک Aggregate تضمین نشده                                   | متوسط                           | دو رویداد یک Aggregate در یک میلی‌ثانیه می‌توانند وارونه به Partition برسند؛ `created_at, id` فقط ترتیب قطعی می‌دهد، نه معنایی | ADR جدا لازم است: `BIGSERIAL` یا `aggregate_sequence` یا پذیرش محدودیت — ADR-050 § Ordering                        |
| D-026 | ادعای رزرو `FOR UPDATE SKIP LOCKED` در Outbox دوام ندارد                         | **بالا**                        | دو نمونه رله می‌توانند یک ردیف را هم‌زمان Claim و در Kafka منتشر کنند (Overlap اندازه‌گیری‌شده: ۱۰ از ۱۰)                      | **Phase B پیاده و تست شد** (PR باز، بازبینی‌نشده). ADR-050 همچنان `Proposed` و این مورد تا پذیرش محصول **باز** است |
...
### D-026 — `FOR UPDATE SKIP LOCKED` رزرو بادوام نمی‌سازد (2026-09-02) 🔴 باز

**یافته بازبینی محصول روی PR #21.** توضیحی که در همان PR نوشته شده بود ادعا
...
شدند؛ **این مورد همچنان باز است** و ADR-050 همچنان `Proposed`. پذیرش پس از
بازبینی محصول روی شواهد PR انجام می‌شود، نه با اتمام کار.
...
- پنج ستون، سه Index و پنج CHECK روی **هر هشت** پایگاه داده؛ `up → down → up`
  روی هر هشت سبز.
- بیست‌وچهار آزمون قطعی روی PostgreSQL واقعی، در Schema ایزوله.

**محدودیت شناخته‌شدهٔ اندازه‌گیری‌شده:** معیار
`rows removed by filter ≤ ۱۰ × LIMIT` در وضعیت‌های «اکثراً Lease فعال» و
«اکثراً در Backoff» نقض می‌شود (۱۹۰٬۰۰۰ در برابر ۱٬۰۰۰). دو Index ترکیبی آزموده
شد و هیچ‌کدام حلش نکرد: گلوگاه **همبستگی** میان واجد شرایط بودن و ترتیب
`created_at` است، نه برآورد گزینش‌پذیری. `Seq Scan` روی هیچ Fixture مقیاس‌داری
رخ نداد و بدترین زمان اندازه‌گیری‌شده ۶۲٫۷ms است. جزئیات در
[طرح اجرا § شواهد Phase B](docs/adr/ADR-050-implementation-plan.md).

**Phase A — طراحی و شواهد (2026-09-02).** هنوز **باز**؛ چیزی پیاده نشده.
...
می‌گفت درحالی‌که ماتریس شانزده‌تا داشت.)
