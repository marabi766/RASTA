...
| `RecordPerformanceEvent` · `GetPerformanceScore` · `performance_score` | ❌ **پیاده نشد.** Q-12 باز است: فرمول و وزن‌ها تصویب نشده‌اند. ستونی که عددی را نگه دارد که کسی نحوهٔ محاسبه‌اش را نپذیرفته، اختراع واقعیت است.   |
| `PERFORMANCE_SCORE_UPDATED`                                            | ❌ **منتشر نمی‌شود** — رویدادی برای عددی که وجود ندارد.                                                                                           |
