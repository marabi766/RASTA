...
| --------------------------------------- | ------------------------------------------------------------------ |
| ترتیب رویدادهای یک دارایی/سفارش/کیف پول | ترتیب **به‌ازای پارتیشن، از یک Producer**؛ کلید = شناسه Aggregate¹ |
...
> ¹ **این ترتیب، ترتیب معنایی نیست.** Kafka ترتیب Append را درون یک Partition و از
> **یک نمونهٔ Producer** تضمین می‌کند. امروز چند Replica از Relay می‌توانند ردیف‌های
> مجزای یک کلید را هم‌زمان منتشر کنند، و ردیفی که در Backoff یا زیر Lease است، ردیف
> بعدیِ همان کلید را جلو می‌اندازد. اندازه‌گیری‌شده با Kafka واقعی: **۸ وارونگی از ۲۰
> آزمون** با دو Relay روی یک Partition. جزئیات و طرح در
> [ADR-051](adr/ADR-051-outbox-semantic-ordering.md) — **`Accepted`**، ولی **پیاده
> نشده**: پذیرش، معماری را تصویب کرد و رفتار امروز را عوض نکرد. مسئله به‌عنوان
> **D-027** باز است.
...
**آنچه امروز پیاده است — `EventConsumer` در `packages/nest-common`:**

```
   Topic اصلی
       │ شکست
       ▼
  Envelope تجزیه/اعتبارسنجی شد؟ ── نه ──► DLQ مستقیم (VALIDATION_FAILED)
       │ بله                              و پارتیشن به راه خود ادامه می‌دهد
       ▼
  Handler شکست خورد ──► Retry درون‌فرآیندی: ۳ تلاش، Backoff خطی ۵۰۰ms × شماره تلاش
       │ تلاش‌ها تمام شد
       ▼
  DLQ (MAX_RETRIES_EXCEEDED)
       │ نوشتن DLQ هم شکست خورد
       ▼
  خطا بالا می‌رود و **پارتیشن متوقف می‌شود** — عمدی
```

| نوع خطا                             | رفتار پیاده‌شده                           |
| ----------------------------------- | ----------------------------------------- |
| قطع شبکه، Timeout، افت موقت وابستگی | Retry درون‌فرآیندی (۳ تلاش، ۵۰۰ms × تلاش) |
| Deadlock پایگاه داده                | همان مسیر Retry درون‌فرآیندی              |
...
> `EventConsumer` تلاش مجدد را درون‌فرآیندی انجام می‌دهد و سپس مستقیم به DLQ می‌رود.
>
> این متن اصلاح شد تا آنچه کد می‌کند را توصیف کند، نه آنچه قصد شده بود. سرویسی که به Retry طولانی‌تر یا پهن‌تری نیاز دارد —
> مثلاً برای قطعی یک ارائه‌دهندهٔ بیرونی — آن را **محلیِ سرویس** پیاده می‌کند، روی ردیف خودش و با `nextAttemptAt`، به‌جای تغییر
> زیرساخت مشترک. نمونهٔ تعیین‌شده در [ADR-054](adr/ADR-054-notification-service-delivery.md) § ۷ است.
...
> **ستون «تضمین» پایین، _نیت_ هر کلید را می‌گوید، نه خاصیتی که امروز برقرار است.**
> کلید Partition تعیین می‌کند رویدادها روی **یک** Partition بنشینند، و همین است که
> ترتیب را _ممکن_ می‌کند. ولی امروز آن را _تضمین_ نمی‌کند: چند Replica از Relay
> می‌توانند ردیف‌های مجزای یک کلید را هم‌زمان منتشر کنند، و Backoff، Lease زنده و
> بازپخش دستی DLQ می‌توانند رویداد بعدی را جلو بیندازند. **تا اجرای**
> [ADR-051](adr/ADR-051-outbox-semantic-ordering.md) — که `Accepted` است ولی هنوز
> **شروع نشده** — این ردیف‌ها را «هم‌Partition بودن» بخوانید، نه «مرتب بودن». مسئله
> **D-027** است و باز می‌ماند.

| دامنه                    | کلید پارتیشن            | نیتِ ترتیب (امروز تضمین‌نشده — D-027)                      |
| ------------------------ | ----------------------- | ---------------------------------------------------------- |
...
> **دربارهٔ `aggregateVersion`:** این فیلد در Envelope **اختیاری** است و امروز فقط
> سه محل تولیدی آن را می‌فرستند — `asset`، `identity` و `organization`. پنج سرویس
> دیگر هرگز نمی‌فرستند، پس تکیه بر آن به‌عنوان ابزار عمومی ترتیب، امروز کار نمی‌کند.
> ADR-051 نشان می‌دهد واحد درست هم `topic + partitionKey` است نه Aggregate، چون
> `fleet`، `maintenance` و بخش تراکنشی `economic` عمداً رویدادهای چند Aggregate را
> روی یک کلید می‌نشانند.
...
> دریافت، اعتراض و حل آن، لغو و بازپرداخت؛ چه کسی، کِی، از کدام وضعیت به کدام، در
> همان تراکنش پایگاه داده. تسویه همچنان `SETTLEMENT_COMPLETED` است. متن آزاد دلیل
> روی Wire نمی‌رود و فقط روی ردیف می‌ماند (S-09). ممیزی اقتصادی دور دوم، بند b.
