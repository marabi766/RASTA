...
| Timeout تأیید دریافت (۶) | ~~تأیید خودکار~~ → **یادآور ثبت می‌شود و سفارش منتظر می‌ماند** (ADR-043، Q-11) |
...
**ASSUMPTION.** مهلت‌های ۷ روز و ۳ روز فرضیه‌اند؛ سند محصول عددی نداده. هر دو
**پیکربندی سازمانی**اند (`MARKETPLACE_FULFILLMENT_WINDOW_DAYS` و
`MARKETPLACE_RECEIPT_WINDOW_DAYS`).

> **اصلاح (2026-08-30، ADR-043).** ردیف «تأیید خودکار» در جدول بالا با تصمیم
> صاحب محصول روی Q-11 نقض شده بود و اکنون اصلاح شده است. **انقضا هیچ پولی را
> حرکت نمی‌دهد**: نه تسویه می‌کند، نه لغو، نه بازمی‌گرداند. یک ردیف یادآور در
> `order_status_history` نوشته می‌شود، شمارنده `rasta_marketplace_orders_overdue`
> بالا می‌رود، و سفارش **نامحدود** منتظر می‌ماند. تأیید خودکار یعنی آزادسازی پول
> بدون رضایت صریح خریدار، و پلتفرم اجازه ندارد سکوت را رضایت بخواند.

**وضعیت پیاده‌سازی (2026-08-30).** `OrderSagaWorkflow` نوشته و اجرا شده است، روی
صف اختصاصی `rasta-order` (ADR-039). مرحله ۳ (`inventory.reserveStock`) و مرحله ۴
(`notifySupplier`) **موکول و نام‌دار**اند و در Query ‏`status` به‌عنوان
`deferredSteps` دیده می‌شوند — سرویس‌هایشان وجود ندارند (ADR-041).
...
> **وضعیت مسیر Activity (2026-08-29).** فراخوانی `economic-service` به‌صورت
> Activity تا امروز **کار نمی‌کرد**: نگهبان مشترک برای فراخوان سرویسی هیچ
> تنانتی تفکیک نمی‌کرد و هر تماس روی نخستین خواندن مستأجر-محدود با `500`
> می‌افتاد (Q-28). با ADR-035 این مسیر باز شد — توکن داخلی یک Claim امضاشده
> `org_id` دارد و سرویس تنانت را از همان می‌گیرد. مسیر کامل
> `Hold → تأیید → تسویه` از یک فراخوان سرویسی در
> `tests/e2e/specs/economic/06-service-to-service.e2e-spec.ts` زنده اجرا
> می‌شود. **`OrderSagaWorkflow` از 2026-08-30 نوشته و اجرا شده است** — نخستین
> مصرف واقعی Temporal در این پلتفرم (ADR-039).
