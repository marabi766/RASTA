...
**وضعیت 2026-08-29.** `tests/e2e` اکنون Package ‏`@rasta/e2e` است: یک
`playwright.config.ts` واقعی، یک Global Setup که هر وابستگی را **مثبت** بررسی
می‌کند و با پیام قابل‌اقدام شکست می‌خورد، و **۳۷ سناریو** روی
`api-gateway` + `economic-service` + PostgreSQL + Kafka + توکن واقعی Keycloak.
...
**نیمه Marketplace ردیف ۴ حذف شده، نه شبیه‌سازی‌شده.** قرارداد `ORDER_*` طبق
ADR-032 موکول است، و نوشتن Payload سرویسی که هنوز وجود ندارد یعنی اختراع
قرارداد آن (`AGENTS.md` § ۹).
