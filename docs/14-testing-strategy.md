...
**مشاهدهٔ دوم و تنگ‌تر زیر ۰٫۰۱۵ CPU — Suite شکست خورد و داخل کران تمام شد (2026-09-16).** بازه با دو سر
بی‌استفاده باز مانده بود (۰٫۰۰۵ تمام نشد، ۰٫۰۵ کامل گذشت)، پس یک‌بار **لگاریتمی** نصف شد:
√(۰٫۰۰۵ × ۰٫۰۵) = ۰٫۰۱۵۸ ← `CPU_PERIOD_US=1000000` / `CPU_QUOTA_US=15000`، یعنی **۰٫۰۱۵ CPU**. **باز هم شواهد
§ ۶ نیست** — یک نمونه، بی Probe/Control/جفت/توزیع — و **Artifact آن مسیر جداگانه دارد، پس Artifact ۰٫۰۵ دست
نخورد**. Workflow موقت و فقط‌شاخه‌ای `aggregation-stress-cost-observation.yml` (مشتق از همان Workflow
بازبینی‌شدهٔ `62062f3`؛ تنها تفاوت‌ها سهمیه، نام Artifact، گروه Concurrency و نام Job) روی Commit ‏`5941943`،
اجرای `35045042687` (تلاش ۱، `push`، Workflow و Job هر دو `failure` — **که همان نتیجهٔ اندازه‌گیری است**)،
همان توپولوژی (Runner بومی Ubuntu یک‌بارمصرف، **دقیقاً یک** Container ‏`postgis/postgis:16-3.4`، مهلت Job ۶۰
دقیقه، کران فرایند ۳۰ دقیقه). گام‌های ۱..۱۳ `success` — از جمله `test:aggregation-evidence-lib` (**۵۲/۵۲**) و
`check:test-phases` **پیش از** سهمیه، و تأیید دقیق `applied_cpu_period_us=1000000` /
`applied_cpu_quota_us=15000` — گام ۱۴ (اندازه‌گیری) `failure`، گام ۱۵ (Upload) `success`؛ **پس هیچ خطای
Setup/قرارداد/سهمیه/Wrapper/Artifact رخ نداد.** **نتیجه
([`docs/evidence/adr-055/aggregation-stress-cost-observation-0015cpu-github-2026-09-16.txt`](evidence/adr-055/aggregation-stress-cost-observation-0015cpu-github-2026-09-16.txt)):**
`outcome=completed`، `timed_out=no`، `exit_status=1`، زمان دیوار **۱۵۳٫۲ ثانیه** (کران ۱۸۰۰ — یعنی **داخل کران،
نه سانسورشده**)، **۰/۱/۰/۱** Suite و **۲۱/۱/۰/۲۲** تست، و طبقه‌بندی شکست **`sqlstate57014=1`** با شش دستهٔ دیگر
صفر. `other=3` در دسته‌های زیرساختی **تشخیص محیطی نیست**: `summarizeJestReport` برای گزارشی که وجود دارد و
می‌گوید تست شکست خورده عمداً هیچ دستهٔ زیرساختی ثبت نمی‌کند و سه مشکل محصولی‌اش در `other` شمرده می‌شوند. یعنی
**۰٫۰۱۵ CPU نخستین شرطی است که هم شکست می‌دهد و هم داخل کران تمام می‌شود**، و شکل شکست همان است که § ۶ از
«شکستِ ناشی از محیط» می‌خواهد (`57014` = `canceling statement due to statement timeout`). **ولی شکست حاشیه‌ای
است (۱ از ۲۲ تست) و این یک نقطه است**، پس نسبت گذشته/شکست‌خورده در بیست جفت از آن پیش‌بینی نمی‌شود. **بازهٔ
به‌روزشده:** ۰٫۰۰۵ ← تمام نشد · **۰٫۰۱۵ ← شکست در ۱۵۳٫۲ ثانیه** · ۰٫۰۵ ← گذشت در ۸۸٫۰ ثانیه. **پیشنهاد کمپین
جفت‌شده — فقط پیشنهاد، در این تکرار اجرا نشد:** ۱۵۳٫۲ × **ضریب ایمنی نام‌دار ۲٫۰** = ۳۱۰ ثانیه، به‌علاوهٔ
**سهمیهٔ نام‌دار Probe ۷۵ ثانیه** = **۳۸۵ ثانیه بر جفت**؛ ۲۰ جفت = ۷۷۰۰ ثانیه، به‌علاوهٔ Control ۳۰ + Setup ۳۰۰
(اندازه‌گیری‌شده ۶۱ و ۸۶ ثانیه) + حاشیهٔ پایانی ۶۰۰ = **۸۶۳۰ ثانیه (۲ ساعت و ۲۳ دقیقه و ۵۰ ثانیه)**؛ پس سهمیهٔ
ثابت ۰٫۰۱۵ CPU، ۲۰ جفت، **کران گام همان ۳۰ دقیقهٔ دست‌نخوردهٔ Harness** (۱۱٫۷ برابر هزینهٔ اندازه‌گیری‌شده) و
**مهلت Job ۱۸۰ دقیقه**، با ۳۶ دقیقه سرِ آزاد و کاملاً داخل سقف ۳۶۰ دقیقه‌ای Job در GitHub. **هیچ آستانهٔ
توانایی پیشنهاد نشد** و هیچ آستانه، حاشیه، Classifier، Preflight، دروازه، Bypass، Retry، Skip-Green یا
...
**بازبینی حکمرانی آستانه — NO-GO برای انتخاب عدد (2026-09-16).** روی همان دو مجموعهٔ جفت‌شدهٔ موجود یک بازبینی
تحلیلی انجام شد و نتیجه‌اش **انتخاب نکردن** آستانه بود؛ **هیچ رفتار پیاده‌سازی تغییر نکرد**. سند:
...
— یک تحلیل مهندسی، نه Artifact تولیدشدهٔ Harness؛ هیچ اندازه‌گیری تازه‌ای نگرفت، هیچ کمپینی اجرا نشد و هیچ
Artifact موجودی را تغییر نداد. همهٔ اعدادش **از سطرهای خودِ Artifactها بازمحاسبه شده‌اند** و با خطوط تجمیعی
همان Artifactها مطابق درآمدند. **چرا NO-GO:** پیش‌شرط **جمع‌آوری** § ۶ برآورده است، ولی پیش‌شرط **حکمرانی**
نه — مجموعهٔ القایی فقط **یک** جفت شکست‌خورده دارد و آن جفت کاملاً با ترتیب اجرا **هم‌آمیخته** است: `pair-1`
هم‌زمان تنها `FAIL`، کندترین `probe_tps` (**۳۹٫۸۲** در برابر بعدی ۱۹۲٫۷۵) و کمترین `probe_min_interval_tps`
(**۱** در برابر بعدی ۳) و بلندترین `stress_wall_s` (**۲۷۱٫۳** در برابر بعدی ۱۵۰٫۴) است. **ممیزی جداپذیری:**
روی `probe_tps` هر `T` با `39.82 < T <= 192.75` این بیست مشاهده را **کاملاً یکسان** جدا می‌کند (بازهٔ باز به
پهنای ۱۵۲٫۹۳، پس داده هیچ راه اصولی برای انتخاب عدد یا حاشیه‌اش نمی‌دهد)؛ روی `probe_min_interval_tps` هر `T`
با `1 < T <= 3` جدا می‌کند ولی فاصلهٔ مشاهده‌شده فقط **۲ واحد** و بی هیچ تکرار است؛ `probe_longest_stall_s`
در هر چهل سطر هر دو کمپین **صفر** است و هیچ قدرت تفکیکی ندارد؛ و کمپین آرام با بیست جفت گذشتهٔ دیگرش
(`probe_tps` ۲۴۹۴٫۸…۳۷۹۷٫۵۱، `probe_min_interval_tps` ۴۰۵…۲۱۷۴٫۳) حدود **۱۱٫۸ برابر** دورتر از مرز است و
انتخاب درون هیچ‌یک از دو بازه را حل نمی‌کند. **آمار توصیفی با چارک Type 7 و بی فرض نرمال بودن** در سند آمده
(القایی `probe_tps`: کمینه ۳۹٫۸۲، Q1 ۲۰۱٫۵۹۲۵، میانه ۲۰۴٫۶، Q3 ۲۰۸٫۹۹، IQR ۷٫۳۹۷۵، بیشینه ۲۱۲٫۰۲، میانگین
۱۹۶٫۹۵۲۵ — میانگینِ زیرِ Q1 یعنی چولگی شدید از یک مقدار پرت). بازهٔ Clopper–Pearson ۹۵٪ برای ۱ از ۲۰
(**[۰٫۱۳٪ ، ۲۴٫۸۷٪]**) فقط **تصویرسازی و صریحاً غیرتصمیمی** است، چون جفت‌های سریال روی یک پایگاه داده استقلال
اثبات‌شده ندارند. **یک نکتهٔ دقت، نه ناسازگاری:** Harness زمان دیوار را در سطر با یک رقم اعشار و در خط توزیع با
دو رقم از همان مقدار گرد‌نشده چاپ می‌کند، پس ۱۱۰٫۶/۱۱۶٫۱/۲۷۱٫۳ (بازمحاسبه از سطرها) و ۱۱۰٫۵۶/۱۱۶٫۱۲/۲۷۱٫۲۷
(خط توزیع) یکدیگر را نقض نمی‌کنند. **قابلیت مقایسه:** خط `topology:` هر دو Artifact بایت‌به‌بایت یکسان است و
`scripts/aggregation-evidence-lib.mjs` و Spec فشار بین دو Commit کمپین **تهی-Diff** اند؛ تنها
`scripts/aggregation-evidence.mjs` با `f2406f4` عوض شده بود و هیچ سطری در هیچ کمپین به کران فرایند نخورد.
**تصمیم:** ADR-055 همچنان `Proposed`؛ هیچ آستانه، حاشیهٔ ایمنی، `VALID_CAPABLE`/`VALID_INCAPABLE`، Classifier
توانایی، Preflight فاز، دروازهٔ Fail-Fast، Bypass، Retry، Skip-Green، سیاست زمان اجرا یا یکپارچه‌سازی CI
اضافه نشد؛ `pair-1` همچنان `VALID` است؛ AUD-004 باز، ADR-053 `Proposed`، `COM-009` ‏`READY`/۱۳.
**گام بعد — فقط ثبت شد، اجرا نشد:** کمپین آیندهٔ **جداگانه بازبینی‌شده** از **تکرارهای مستقلِ جفتِ نخست** روی
همان شرط `CPU_PERIOD_US=1000000` / `CPU_QUOTA_US=15000`، با **یک جفت در هر Job و پایگاه دادهٔ یک‌بارمصرفِ
تازه** (`--pairs 1`)، برای اینکه معلوم شود شکستِ `pair-1` و Probe پایینش در اجراهای تازه **تکرار می‌شود یا یک
Artifact ترتیب سریال بوده است**؛ با تأیید دقیق تصویر/توپولوژی/سهمیه، ثابت‌ها و Timeoutهای دست‌نخورده، بی
Retry، Artifact تجمیعیِ Redactشده، حفظ خروج غیرصفر و بی یکپارچه‌سازی CI. **تعداد تکرار اختراع نشد** — تعیین
حجم نمونه پیش‌نیاز صریح آن بازبینی است و باید از یک هدف دقت/خطای بیان‌شده مشتق شود؛ و اگر تکرار کافی نبود، یک
طراحی **Steady-State/نزدیکِ مرز** همچنان لازم خواهد بود. قاعدهٔ Fallback نصف‌سازی لگاریتمی **کاربرد ندارد**،
چون آن برای مجموعهٔ یک‌طرفه بود و این مجموعه دوطرفه است.

**طراحی Pre-Register شدهٔ تکرار جفتِ نخست در اجرای تازه — فقط طراحی، اجرا نشد (2026-09-16).** بازبینی حکمرانی
آستانه یک پیش‌نیاز صریح گذاشته بود: پیش از هر کمپین تکرار، هدف آماری قابل دفاع بیان و تعداد تکرار از آن **مشتق**
شود. آن پیش‌نیاز اکنون در یک **Preregistration** برآورده شده است:
...
**هیچ کمپینی اجرا نشد، هیچ اندازه‌گیری تازه‌ای گرفته نشد، هیچ Workflowی ساخته نشد و رفتار تست اینجا تغییر
نکرد.** هدف: کوچک‌ترین احتمال تکرارِ قابل تشخیص **۵٪** (تنها به این دلیل که `1/20 = 5%` یگانه فراوانی شکست
مشاهده‌شده در کمپین القایی است — نه تخمین نرخ واقعی، نه تحمل کسب‌وکاری، نه آستانه) با اطمینان **یک‌طرفهٔ ۹۵٪**.
تعداد ثابت از همان دو عدد مشتق می‌شود: `1 - 0.95^n >= 0.95` ⟵ `n >= log(0.05)/log(0.95) = 58.4039748143197` ⟵
**`n = 59`** (در `n=58` توان ‏`94.895313%` است و کافی نیست؛ در `n=59` ‏`95.150547%`). اگر هر ۵۹ تکرار معتبر
باشند و هیچ شکست واجد شرایطی رخ ندهد، کران بالای Clopper–Pearson یک‌طرفهٔ ۹۵٪ برابر `1 - 0.05^(1/59) ≈ 4.9508%`
است. ۵۹ Slot همه اجرا می‌شوند (بی توقف اختیاری، بی Retry، بی جایگزینی)؛ یک جفت در هر Job و پایگاه دادهٔ
یک‌بارمصرفِ تازه با `--pairs 1` روی همان شرط `CPU_PERIOD_US=1000000` / `CPU_QUOTA_US=15000`؛ و اگر حتی یک Slot
نامعتبر یا غایب باشد، کمپین برای هدف ۵٪/۹۵٪ **INCONCLUSIVE** است. مهلت Job **۴۵ دقیقه** از شواهد اندازه‌گیری‌شده
بازمحاسبه شد (هزینهٔ مورد انتظار ≈۴۲۵ ثانیه، بدترین حالت کراندار ۲۳۰۸ ثانیه)، نه از ضرب بودجهٔ ۲۰ جفتی. آستانه،
حاشیهٔ ایمنی، Predictor، Classifier، Preflight فاز و دروازهٔ CI **همچنان موکول‌اند**.
