...
**یک مشاهدهٔ هزینه زیر ۰٫۰۵ CPU — Suite گذشت، پس شرط بی‌فایده بود (2026-09-15).** برای بستن مسدودکنندهٔ (۱)
هزینهٔ **یک** اجرای دست‌نخوردهٔ Suite زیر یک شرط محدودکننده برای نخستین بار اندازه گرفته شد. **این شواهد § ۶ نیست**
— نه نمونه، نه جفت، نه توزیع، نه سمت شکست. Workflow موقت و فقط‌شاخه‌ای `aggregation-stress-cost-observation.yml`
روی Commit ‏`62062f3`، اجرای `35026098062` (تلاش ۱، Workflow و Job هر دو `success`)، همان توپولوژی کمپین آرام
(Runner بومی Ubuntu یک‌بارمصرف، **دقیقاً یک** Service Container ‏`postgis/postgis:16-3.4`، بی هر سرویس دیگر،
مهلت Job ۶۰ دقیقه). **شرط:** پس از Provisioning/Build/Migration، همان یک Container با فیلتر دقیق تصویر یافت شد و
`CPU_PERIOD_US=1000000` / `CPU_QUOTA_US=50000` — **۰٫۰۵ CPU**، ده برابر سست‌تر از شرط ناکارآمد ۰٫۰۰۵ — با
`docker update` روی آن نشست و با `docker inspect --format` دو‌فیلدی **دقیقاً** تأیید شد. پیش از سهمیه
`test:aggregation-evidence-lib` (**۵۲/۵۲** روی Ubuntu بومی) و `check:test-phases` گذشتند؛ پس از آن **دقیقاً یک**
فرایند تازهٔ بی‌فیلتر `pnpm run test:aggregation-stress` از راه همان `runBounded` صادرشده با `LAUNCHER.pnpm` و کران
سخت ۳۰ دقیقه‌ای روی کل درخت فرایند اجرا شد — بی Probe، بی Control، بی جفت، بی Retry، و بدون تکرار منطق Process
Group در Shell. **نتیجه ([`docs/evidence/adr-055/aggregation-stress-cost-observation-github-2026-09-16.txt`](evidence/adr-055/aggregation-stress-cost-observation-github-2026-09-16.txt)):**
`outcome=completed`، `timed_out=no`، `exit_status=0`، زمان دیوار **۸۸٫۰ ثانیه**، **۱/۰/۰/۱** Suite و
**۲۲/۰/۰/۲۲** تست، و هر هفت دستهٔ شکست و هر پانزده دستهٔ تشخیص زیرساختی **صفر**. یعنی **۰٫۰۵ CPU شرط مفیدی برای
سمت شکست نیست**: اثبات زیر آن کامل گذشت. مقایسهٔ محتاطانهٔ **یک نمونه** با توزیع بیست‌نمونه‌ای کمپین آرام — ۸۸٫۰
در برابر میانهٔ ۵۷٫۶۴ ثانیه روی چهار CPU بی‌قید — یعنی حدود ۱٫۵ برابر کندتر در ازای سهمیه‌ای ~۱/۸۰ آن CPU؛
برداشت محتمل، کران‌داری با تأخیر و سریال‌شدن است نه توان CPU، ولی از **یک** نقطه چیزی اثبات نمی‌شود. تنها دستاورد
قابل استناد، **کراندار شدن جست‌وجو** است: شرط قابل استفاده — که هم شکست بدهد و هم داخل کران تمام شود — میان ۰٫۰۰۵
(در ۱۷۷ دقیقه تمام نشد) و ۰٫۰۵ (در ۸۸ ثانیه گذشت) است. **گام بعدی پیشنهادی باز هم یک مشاهدهٔ تکی است، نه کمپین:**
همان طراحی با `CPU_QUOTA_US=15000` (**۰٫۰۱۵ CPU**، میانگین هندسی بازه). سقف زمانی یک اجرای کاملاً Timeout-خورده،
بازمحاسبه از خودِ Spec: ۱۸×۶۰ + ۳×۱۲۰ + ۱×۹۰ = ۱۵۳۰ ثانیه به‌علاوهٔ `beforeAll`/`afterAll` هرکدام ۶۰ ثانیه =
**۱۶۵۰ ثانیه (۲۷ دقیقه و ۳۰ ثانیه)** — زیر کران ۳۰ دقیقه، ولی تنها با ۱۵۰ ثانیه حاشیه و بدون حساب کار معلقی که پس
از شلیک Timeout زنده می‌ماند. **هیچ پارامتر کمپینی (سهمیهٔ نهایی، تعداد جفت، کران گام، مهلت Job) انتخاب نشد** و
هیچ آستانه، حاشیه، Classifier توانایی، Preflight فاز، دروازهٔ Fail-Fast، Bypass، Retry، Skip-Green یا
یکپارچه‌سازی CI اضافه نشد؛ `ci.yml`، فازهای تست، `pnpm verify`، Spec، Probe و Harness دست نخوردند و Workflow موقت
**در همان تکرار حذف شد**. الزام سمت شکستِ § ۶ همچنان **برآورده نشده** است.
