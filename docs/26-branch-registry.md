...
**آخرین به‌روزرسانی:** ۱۴۰۵/۰۷/۰۱ (2026-09-23) · **`main` در آن لحظه:** `d7500be`
...
| `fix/identity-getuser-fail-closed-and-exp` | `F:\Rasta-Parallel\web-organizations-surface` | Opus | #87 | ۱ / ۰          | از `origin/main` @ `d7500be` — سه یافتهٔ مستقل از طرح ADR-060: `getUser` با شکست بسته، `PATCH` غیرخود فقط با `ORGANIZATION_ADMIN`، `exp` الزامی؛ **`packages/nest-common` و `apps/web/src/server/oidc.ts` را لمس می‌کند**                                                                                                                                                                                                 |
...
| `claude/upbeat-tesla-cenkph`               | نشست ابری Claude Code                         | Opus | —   | —              | از `origin/main` @ `cfdb7b0` — کار PM «`chore/gates-service-tests`» (PR ۱ از ۳ گسترش دروازه‌های ایستا): `tsconfig.test.json` در هر سرویس و بسته تا `src/**/*.spec.ts`، `test/**/*.ts` و `prisma/seed.ts` هم Type-Check شوند؛ دامنهٔ Lint سرویس‌ها به `test` و `prisma/seed.ts` گسترش می‌یابد؛ **`package.json` همهٔ سرویس‌ها/بسته‌ها و `pnpm-lock.yaml` را لمس می‌کند**. نام شاخه را محیط نشست تعیین کرده، نه قرارداد § ۶ |
