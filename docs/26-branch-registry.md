...
| `fix/identity-tenant-bound-roles` | `F:\Rasta-Parallel\web-organizations-surface` | Opus | —   | ۰ / ۰          | تازه ساخته شد از `origin/main` @ `cfdb7b0` — **نقش‌ها سراسری‌اند، نه به‌ازای سازمان** (بحرانی)؛ `ADR-060` و `Q-62` را می‌برد. **`packages/nest-common` (گارد مشترک همهٔ سرویس‌ها)، `identity-service` و تعریف Realm را لمس می‌کند**؛ از `services/*/tsconfig` و `economic-service` دور می‌ماند (نشست ابری). پوشهٔ Worktree دوباره استفاده شد                                                                                                                                                                                                                                                                                                                                                                       |
...
| `claude/upbeat-tesla-cenkph`      | نشست ابری Claude Code                         | Opus | —   | —              | PR #86 ادغام شد (`4ced7f3`)؛ شاخه از `origin/main` @ `4ced7f3` از نو ساخته شد و `fix/identity-tenant-bound-roles` (#84، متن `ADR-060`) در آن ادغام شد، برای کار PM «ADR-060 PR A» (Projection عضویت‌ها به Keycloak): User Profile اعلامی و Mapper `org_roles` در Realm، `KeycloakProjector` در `identity-service`، مصرف‌کنندهٔ بازProjection و فرمان‌های Backfill/Reconcile، اجرای `validUntil` عضویت و Scanner انقضا (Migration `membership.lapse_handled_at`، رویداد `MEMBERSHIP_EXPIRED`)، آزمون E2E با Keycloak زنده. **`infrastructure/docker/keycloak/rasta-realm.json`، `identity-service` و `tests/e2e` را لمس می‌کند؛ گارد `packages/nest-common` دست نمی‌خورد (PR B).** نام شاخه را محیط نشست تعیین کرده |
...
| `claude/upbeat-tesla-cenkph`             | [#86](https://github.com/marabi766/RASTA/pull/86) | `4ced7f3`   |
| `feat/ci-portal-e2e-live-stack`          | [#78](https://github.com/marabi766/RASTA/pull/78) | `39ac75c`   |
...
| ۰۶۰   | `fix/identity-tenant-bound-roles`     | **رزرو شد** (2026-09-24) — نقش‌های وابسته به سازمان                  |
| ۰۶۱   | `docs/audit-trail-provenance`         | **رزرو شد** (2026-09-24) — اصالت تولیدکننده در زنجیرهٔ Audit (Kafka) |
| ۰۶۲+  | آزاد                                  | —                                                                    |
...
| Q-62        | `claude/upbeat-tesla-cenkph`             | سیاست بازپرداخت در `economic-service` (#86) — **همین شماره را این شاخه هم برداشته بود**؛ #86 زودتر ادغام می‌شود و آن را نگه می‌دارد |
| Q-63        | `fix/identity-tenant-bound-roles`        | **رزرو شد** (2026-09-24) — کدام نقش‌ها در همهٔ سازمان‌ها معتبرند. ابتدا Q-62 بود؛ با #86 تصادم کرد و بازشماری شد                    |
| Q-64+       | آزاد                                     | —                                                                                                                                   |
