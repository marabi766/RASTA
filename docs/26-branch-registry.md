...
| ---------------------------------------- | ------------------------------------------------------ | ------- | ---- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fix/fleet-dispatch-and-usage-integrity` | `F:\Rasta-Parallel\fleet-dispatch-and-usage-integrity` | Opus #2 | #103 | ۶ / ۰          | دستهٔ یکپارچگی دامنه (`F:\Rasta-audit\L3-domain.md`)، PR ۱ از ۲: `L3-02` (مانع اعزام به‌ازای هر Cause و بیمه به‌ازای هر پوشش؛ `coverage` در `INSURANCE_EXPIRED`؛ **Q-65**)، `L3-05` (بازه‌های مصرف هم‌پوشان رد می‌شوند)، `L3-11` (`DRIVER_UPDATED` و `REPAIR_CANCELLED`). از Sonnet تحویل گرفته شد. `fleet`، `maintenance` و `asset` را لمس می‌کند |
