...
| `claude/quirky-curie-15d850` | `F:\Rasta\.claude\worktrees\quirky-curie-15d850` | —    | —   | ۱ / ۰          | حذف `--passWithNoTests` از `test:integration` در `identity` و `organization`، به‌روزرسانی یادداشت کهنهٔ `docs/14`، و دروازهٔ ایستای تازه در `check:test-phases`. **با `chore/test-task-unit-only` روی همان دو فایل `scripts/test-phases-*` تداخل متنی دارد** — § ۲۶٫۴ |
| `chore/test-task-unit-only`  | (نشست همتا)                                      | —    | —   | ۱ / ۰          | فقط محلی، هنوز Push نشده — `test` هر سرویس را Unit-only می‌کند و `validateInfraFreeTestTask` را می‌افزاید؛ ردیفش را نشست صاحبش کامل کند                                                                                                                               |
...
| `scripts/test-phases-lib.mjs` و `check-test-phases.mjs`     | هر قاعدهٔ تازهٔ تست یک Validator تازه و یک ورودی تازه در `readTestPhaseInputs` می‌خواهد؛ دو شاخه همین حالا هر دو را لمس می‌کنند | `claude/quirky-curie-15d850` و `chore/test-task-unit-only` |
