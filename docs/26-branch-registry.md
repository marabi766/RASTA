...
| `fix/identity-registration-approval-oracle` | `F:\Rasta-Parallel\identity-registration-approval-oracle` | Sonnet | —   | ۰ / ۰          | تازه ساخته شد از `origin/main` @ `1a331e1` — دو نقص در `identity-service`: (۱) `approveRegistration` یک Oracle بود — جستجوی سراسری‌پلتفرم پیش از بررسی محدودهٔ Provisioning، پس یک `UNION_ADMIN` سازمان A می‌توانست با کد وضعیت (۴۰۴/۴۰۹/۴۰۳) بفهمد آیا شناسهٔ ثبت‌نامی در سازمان B وجود دارد و در چه وضعیتی است. (۲) `rejectRegistration` اصلاً بررسی محدوده نداشت — همان `UNION_ADMIN` می‌توانست ثبت‌نام هر سازمانی را واقعاً رد کند، نه فقط کاوش کند. هر دو با یک کمکی مشترک (`isWithinProvisioningScope`) رفع شدند. فقط `identity-service` را لمس می‌کند |
