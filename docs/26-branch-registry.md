...
**آخرین به‌روزرسانی:** ۱۴۰۵/۰۶/۲۷ (2026-09-18) · **`main` در آن لحظه:** `cc89660`
...
> (در زمان نگارش، ۱۶۱ کامیت). هیچ Worktreeای `main` را checkout نکرده، پس
> این ref به‌خودی‌خود به‌روز نمی‌شود. **همیشه `origin/main` را مبنا بگیرید،
> نه `main`.**
...
| ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `docs/24-open-questions.md`                                 | هر شاخه پرسش تازه‌اش را ته همین فایل می‌افزاید و جدول § ۲۴٫۱۰ را بازمی‌شمارد. **منبع هر سه حادثهٔ § ۲۶٫۱.** | C3، و `docs/audit-immutable-archive` |
...
| `scripts/verify-migration-reversible-lib.mjs` و `.test.mjs` | هر سرویس یک بخش تازه می‌افزاید (معمولاً افزایشی و بی‌دردسر)                                                 | C2، C3                               |
| `pnpm-lock.yaml`                                            | هر نصب بازتولیدش می‌کند                                                                                     | هر شاخه‌ای که وابستگی اضافه کند      |
| `PROJECT_MEMORY.md`                                         | همه بعد از ادغام می‌خواهند به‌روزش کنند                                                                     | `docs/audit-immutable-archive`       |
...
| Q-56        | `feat/supplier-performance-phase2`    | **رزرو**، هنوز ادغام نشده                                                              |
