...
| `fix/document-upload-immutability` | `F:\Rasta-Parallel\document-upload-immutability` | —    | —   | ۰ / ۰          | **P0 امنیتی، تأییدشدهٔ PM** — تازه ساخته شد از `origin/main` @ `d7500be`: پس از `CLEAN` شدن اسکن، سند از راه URL بارگذاری هنوز معتبرش قابل بازنویسی است، بدون تغییر وضعیت `CLEAN`؛ کلید بارگذاری‌شده و کلید سرو/اسکن‌شده یکی نیستند در اثبات. `services/document-service` را لمس می‌کند                                                                                                                                   |
