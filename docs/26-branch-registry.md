...
| `fix/economic-source-verification`       | `F:\Rasta-Parallel\economic-source-verification`       | Opus #2 | —    | ۰ / ۰          | تازه ساخته شد از `origin/main` @ `b55e4e9` — ADR-061 PR ۱ (بند ۴): `settlement-authority` و `reward-trigger` در economic-service واقعیت را از سرویس مالک می‌پرسند؛ Endpointهای داخلی تازه در `maintenance` و `fleet`؛ `UnprocessableEventError` و `SOURCE_UNCONFIRMED` در `nest-common`/`contracts`؛ قاعدهٔ A-13 در `AGENTS.md`                    |
