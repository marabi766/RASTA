...
| `feat/web-drivers-surface`      | `F:\Rasta-Parallel\web-drivers-surface`       | —    | —   | ۰ / ۰          | تازه ساخته شد از `origin/main` @ `0b4a35a` — `EXP-002`: `/drivers` (فهرست، جزئیات، ثبت، ویرایش، تغییر وضعیت، تخصیص/پایان تخصیص)؛ **`nav.ts` را لمس می‌کند** — Opus هم‌زمان `/organizations` می‌سازد |
