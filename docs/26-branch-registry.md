...
| -------------------------------------------- | ---------------------------------------------------------- | ------- | ---- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `fix/asset-transfer-and-insurance-integrity` | `F:\Rasta-Parallel\asset-transfer-and-insurance-integrity` | Opus #2 | #108 | ۶ / ۰          | دستهٔ یکپارچگی دامنه، PR ۲ از ۲ (`asset-service`): `L3-03` (انتقال با تخصیص، تعمیر یا خسارت باز رد می‌شود)، `L3-07` (CAS وضعیت؛ `DECOMMISSIONED` احیا نمی‌شود)، `L3-08` (کل پرونده، بیمه و معاینه هم، با دارایی منتقل می‌شود)، `L3-09` و `L3-10` (Unique جزئی؛ نرمال‌سازی شناسه‌ها)، `L4-03` (Marker و تغییر وضعیت در یک تراکنش)؛ **Q-66**. `L3-04` (Sweep انقضای بیمه) و قفل/بازبینی مالک در `recordPolicy`/`recordInspection` پس از ادغام #103 |
...
| Q-66        | `fix/asset-transfer-and-insurance-integrity` | رزرو — آیا بیمه‌نامهٔ معتبرِ مالک قبلی پس از انتقال، پوشش مالک جدید است (Q-65 را #103 گرفته است)                              |
| Q-67+       | آزاد                                         | —                                                                                                                             |
