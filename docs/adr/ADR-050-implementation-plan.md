...
`LIKE public.outbox_message INCLUDING ALL` (یعنی دقیقاً همان Index ها و
CHECK هایی که Migration ساخت). `LIMIT 100`.

| #   | وضعیت              | ردیف      | اندازه جدول | Plan انتخابی Planner        | Index                 | rows removed   | زمان اجرا    | shared buffers        |
| --- | ------------------ | --------- | ----------- | --------------------------- | --------------------- | -------------- | ------------ | --------------------- |
| ۱   | صف خالی            | ۰         | 8192 bytes  | `Seq Scan` (never executed) | —                     | ۰              | **0.123 ms** | hit=39 (فقط Planning) |
| ۲   | اکثراً Claimنشده   | ۲۰۰٬۰۰۰   | 24 MB       | `Index Scan`                | `ix_outbox_claimable` | **۰**          | **6.85 ms**  | hit=1937              |
| ۳   | اکثراً Lease فعال  | ۲۰۰٬۰۰۰   | 28 MB       | `Index Scan`                | `ix_outbox_claimable` | **۱۹۰٬۰۰۰** ❌ | **58.1 ms**  | hit=6428              |
| ۴   | اکثراً در Backoff  | ۲۰۰٬۰۰۰   | 26 MB       | `Index Scan`                | `ix_outbox_claimable` | **۱۹۰٬۰۰۰** ❌ | **62.7 ms**  | hit=5988              |
| ۵   | اکثراً Lease منقضی | ۲۰۰٬۰۰۰   | 28 MB       | `Index Scan`                | `ix_outbox_claimable` | **۰**          | **4.62 ms**  | hit=2035              |
| ۶   | Backlog ۱ میلیونی  | ۱٬۰۰۰٬۰۰۰ | 120 MB      | `Index Scan`                | `ix_outbox_claimable` | **۰**          | **4.49 ms**  | hit=1926 read=11      |

**وضعیت ۱ طبق معیار تفکیک‌شده پذیرفته است:** رابطه تهی (۰ صفحه، ۰ ردیف)، زیر
یک میلی‌ثانیه، و زیرشاخهٔ `Seq Scan` اصلاً اجرا نشد. اعداد واقعی بالا ثبت شده‌اند.

**`Seq Scan` روی هیچ Fixture مقیاس‌داری رخ نداد** — شرط سخت رد شدن برقرار نشد.

### نقض معیار `rows removed by filter ≤ ۱۰ × LIMIT` در وضعیت‌های ۳ و ۴

این دقیقاً همان ریسکی است که خود ADR پیش‌بینی کرده بود: Index ترتیب را می‌دهد،
واجد شرایط بودن را نه. ۱۹۰٬۰۰۰ در برابر سقف ۱٬۰۰۰.

**دو Index ترکیبی آزموده شد و هیچ‌کدام مسئله را حل نکرد:**

1. `((GREATEST(COALESCE(claim_expires_at,'-infinity'), COALESCE(next_attempt_at,'-infinity'))), created_at, id) WHERE published_at IS NULL`
   — Planner انتخابش نکرد؛ برآوردش برای عبارت، پیش‌فرض ۳۳٪ ماند (rows=66667).
   زمان اجرا بدتر شد: ۱۶۰ms در وضعیت ۳.
2. `(claim_expires_at NULLS FIRST, created_at, id)` و
   `(next_attempt_at NULLS FIRST, created_at, id)`، هر دو `WHERE published_at IS NULL`
   — این‌بار برآورد **درست** شد (rows=9760 ≈ ۵٪)، اما Planner باز هم Index
   ترتیب را برداشت: با `LIMIT 100` هزینهٔ خروج زودهنگام را ارزان می‌بیند.

**تشخیص:** گلوگاه، برآورد گزینش‌پذیری نیست؛ **همبستگی** میان واجد شرایط بودن و
جای ردیف در ترتیب `created_at` است. رله همیشه قدیمی‌ترین‌ها را Claim می‌کند، پس
ردیف‌های دارای Lease دقیقاً قدیمی‌ترین‌هایند و هر پیمایشِ مرتب باید از رویشان رد
شود. Planner راهی برای دانستن این همبستگی ندارد. تا وقتی `ORDER BY created_at, id`
الزام است (G6)، هیچ طراحی Index این را حذف نمی‌کند.

**تصمیم:** طراحی Index بدون تغییر می‌ماند و این به‌عنوان **محدودیت شناخته‌شده**
ثبت می‌شود، نه به‌عنوان یادداشتی که از کنارش رد شده باشد:

- بدترین عدد اندازه‌گیری‌شده **۶۲٫۷ میلی‌ثانیه** است، روی Outbox دویست‌هزارتایی که
  ۹۵٪ آن واجد شرایط نیست. Poll هر ۵۰۰ms است، پس این هنوز جا دارد.
- وضعیت ۳ یعنی ~۱۹۰۰ Batch هم‌زمان زیر Lease زنده (~۱۹۰۰ Worker). وضعیت ۴ یعنی
  ۹۵٪ Outbox مسموم. هیچ‌کدام حالت عادی نیستند.
- Outbox واقعی امروز بیشینه **۴۷۲ ردیف** دارد و `purgePublished` کوچکش نگه می‌دارد.
- اگر پایش نشان داد Outbox بزرگ شده **و** نسبت ردیف‌های واجد شرایط پایین مانده،
  گزینهٔ بعدی رهاکردن ترتیب `created_at` سراسری است — که G6 را لمس می‌کند و
  **ADR خودش را می‌خواهد**، نه یک Index دیگر.

### تصمیم صریح دربارهٔ دو Index دیگر

ADR خواسته بود اگر `ix_outbox_claim_expiry` و `ix_outbox_next_attempt` در هیچ
وضعیتی انتخاب نشدند، تصمیم صریح گرفته شود. **در هیچ‌یک از شش وضعیتِ پرس‌وجوی
Claim انتخاب نشدند.** اما هر دو مصرف‌کنندهٔ واقعی دارند و اندازه‌گیری شد:
...
**تصمیم: هر دو می‌مانند** — نه برای مسیر Claim، بلکه برای رصد. بدون
`ix_outbox_claim_expiry`، Gauge ای که هر ۳۰ ثانیه نمونه‌برداری می‌شود یک
`Seq Scan` کامل می‌شود.
