...
> پیوست [ADR-050](ADR-050-outbox-durable-claim.md).
>
> **این سند اجرا، Merge و پذیرفته شده است.** Phase B روی هر هشت سرویس پیاده
> شد، در PR #23 با CI تأیید شد، به `main` Merge شد (Merge Commit
> `04bc6d8b5d4869538b6675cf5b15a42fb775f893`)، و اجرای کامل CI روی `main`
> ([Run 33754946545](https://github.com/marabi766/RASTA/actions/runs/33754946545))
> با ۱۳ از ۱۳ Job سبز — شامل هر هشت جفت Build/Trivy Image — تأیید شد. صاحب
> محصول روی همین شواهد پذیرش را تأیید کرد: **ADR-050 اکنون `Accepted` است و
> D-026 بسته شد** (2026-09-03). شواهد اندازه‌گیری‌شده در بخش «شواهد اجرا»
> پایین همین سند است. بخش‌های «توالی Commit» و «آزمون‌های الزامی» پایین، طرح
> اولیه‌اند و به‌عنوان طرح تاریخی خوانده می‌شوند، نه به‌عنوان کار باقی‌مانده.
> D-027 (ترتیب معنایی به‌ازای Aggregate) با این پذیرش حل **نمی‌شود** و همچنان
> باز است.
...
**Commit ۱۱ انجام شد ✅ (2026-09-03).** به‌جای یک Commit در همان شاخه،
یک PR جداگانهٔ فقط-مستندات (`docs(architecture): accept durable outbox claim
protocol`) پس از سبزشدن کامل CI روی `main` باز شد و صاحب محصول پذیرش را تأیید
کرد. نتیجه همان است که این ردیف پیش‌بینی کرده بود: D-026 بسته شد و ADR-050
`Accepted` شد.
...
جدول را برای **هر هشت** پایگاه داده پر می‌کرد و در PR می‌گذاشت — **انجام شد؛
اعداد واقعی در [§ Preflight — هر هشت پایگاه داده](#preflight--هر-هشت-پایگاه-داده)
پایین همین سند است.** جدول زیر شکل خواسته‌شده را نشان می‌دهد:
...
می‌گذاشت — **انجام شد؛ نقشه‌های واقعی و اعدادشان در بخش شواهد پایین همین سند
است.** شش وضعیت خواسته‌شده:
...
| ----- | ------------------ | ---------------------------------------------------------- | --------- |
| ۰     | فقط قدیم           | پیش از Phase B — Claim تکراری ممکن                         | ❌        |
...
مرحلهٔ ۲ بدتر از مرحلهٔ ۰ نیست — همان رفتار پیش از Phase B. اما G1 در طول
...
همان Tick نمونه‌برداری `set` می‌شوند. الگوی **قبلی**
`onBatchPublished: (count) => outboxPendingTotal.dec(…)` در `app.module.ts`
دقیقاً همان چیزی بود که پس از Restart یا یک خطای ازدست‌رفته Drift می‌کرد؛
**در هر هشت سرویس به `set` تبدیل شد** (انجام‌شده، نه باقی‌مانده).
...
> این بخش را Phase B پر کرد. **صاحب محصول پذیرش را روی همین شواهد تأیید کرد:
> ADR-050 اکنون `Accepted` است و D-026 بسته شد** (2026-09-03، پس از سبزشدن
> کامل CI روی `main`). D-027 با این پذیرش حل نمی‌شود و همچنان باز است.
...
| #   | وضعیت              | ردیف      | جدول       | صفحه  | Plan                      | Index های انتخاب‌شده                                                    | rows removed | Batch   | قفل/جریان | Sort      | Disk  | Planning | Execution (میانه) |
| --- | ------------------ | --------- | ---------- | ----- | ------------------------- | ----------------------------------------------------------------------- | ------------ | ------- | --------- | --------- | ----- | -------- | ----------------- |
| ۱   | صف خالی            | ۰         | 8192 bytes | ۰     | `Seq Scan` روی رابطهٔ تهی | —                                                                       | **۰**        | ۰       | ۰         | —         | ندارد | 0.629 ms | **0.108 ms**      |
| ۲   | اکثراً Claimنشده   | ۲۰۰٬۰۰۰   | 24 MB      | ۳۰۸۱  | `Index Scan`              | `due_fresh`, `due_lease`, `due_retry`, `next_attempt_at`, `pkey`        | **۰**        | ۱۰۰     | ۱۰۰       | quicksort | ندارد | 0.564 ms | **3.691 ms**      |
| ۳   | اکثراً Lease فعال  | ۲۰۰٬۰۰۰   | 28 MB      | ۳۶۱۳  | `Index Scan`              | `due_fresh`, `due_retry`, `claim_expires_at`, `next_attempt_at`, `pkey` | **۰**        | ۱۰۰     | ۱۰۰       | quicksort | ندارد | 0.815 ms | **3.969 ms**      |
| ۴   | اکثراً در Backoff  | ۲۰۰٬۰۰۰   | 26 MB      | ۳۲۷۳  | `Index Scan`              | `due_both`, `due_fresh`, `due_lease`, `claim_expires_at`, `pkey`        | **۰**        | ۱۰۰     | ۱۰۰       | quicksort | ندارد | 0.566 ms | **3.612 ms**      |
| ۵   | اکثراً Lease منقضی | ۲۰۰٬۰۰۰   | 28 MB      | ۳۶۱۳  | `Index Scan`              | `due_fresh`, `due_retry`, `created_at_id`, `next_attempt_at`, `pkey`    | **۰**        | ۱۰۰     | ۲۰۰       | quicksort | ندارد | 0.691 ms | **4.517 ms**      |
| ۶   | Backlog ۱ میلیونی  | ۱٬۰۰۰٬۰۰۰ | 120 MB     | ۱۵۳۸۹ | `Index Scan`              | `due_fresh`, `due_lease`, `due_retry`, `next_attempt_at`, `pkey`        | **۰**        | ۱۰۰     | ۱۰۰       | quicksort | ندارد | 0.686 ms | **5.103 ms**      |
| قفل | پیشوند قفل‌شده     | ۲۰۰٬۰۰۰   | 24 MB      | ۳۰۸۱  | `Index Scan`              | همان وضعیت ۲                                                            | **۰**        | **۱۰۰** | ۱۰۰       | quicksort | ندارد | —        | **4.821 ms**      |

اعداد Execution میانهٔ چهار اجرای گرم‌اند (اجرای نخست هر وضعیت کنار گذاشته شد).
پراکندگی مشاهده‌شده در ۲۴ اجرای گرم: ۰٫۰۸۷ تا ۱۰٫۶۶۷ میلی‌ثانیه.

**ردیف آخر، Fixture پیشوند قفل‌شده است:** ۲۰۰٬۰۰۰ ردیف واجد شرایط و صد ردیف
قدیمی‌تر که Session دیگری قفل نگه داشته؛ پرس‌وجو **Batch کامل صدتایی بعدی** را
با زمان کران‌دار برمی‌گرداند.
...
### هزینهٔ Index و قفل — یک اندازه‌گیری، ثبت‌شده برای بازبینی

یک‌بار اندازه‌گیری شد، روی Fixture ۲۰۰٬۰۰۰ ردیفی که **هر چهار جریان** را پر
می‌کند (۱۹۴٬۰۰۰ `fresh` / ۲٬۰۰۰ `lease` / ۲٬۰۰۰ `retry` / ۲٬۰۰۰ `paired`):

| اندازه                   | بایت          | خوانا       |
| ------------------------ | ------------- | ----------- |
| جدول                     | ۱۸٬۶۵۳٬۱۸۴    | **18 MB**   |
| Index ها پیش از این فایل | ۲۹٬۱۰۶٬۱۷۶    | **27.8 MB** |
| Index ها پس از این فایل  | ۳۷٬۳۵۵٬۵۲۰    | **35.6 MB** |
| **چهار Index تازه**      | **۸٬۲۴۹٬۳۴۴** | **7.87 MB** |

یعنی **+۲۸٫۳٪** روی حجم Index. تفکیک:

| Index                 | اندازه  |
| --------------------- | ------- |
| `ix_outbox_due_fresh` | 7696 kB |
| `ix_outbox_due_lease` | 120 kB  |
| `ix_outbox_due_retry` | 120 kB  |
| `ix_outbox_due_both`  | 120 kB  |

`ix_outbox_due_fresh` تقریباً همهٔ حجم را دارد، چون تقریباً همهٔ ردیف‌های
منتشرنشده در جریان `fresh`اند و عملاً هم‌پوشان `ix_outbox_claimable` است. **هر
دو نگه داشته شدند:** حذف `ix_outbox_claimable` وضعیت ۵ را (که Planner همان را
برای جریان `lease` برمی‌دارد) و `oldestPendingAgeSeconds()` را بدتر می‌کند.

**چرا اندازه‌گیری قبلی `ix_outbox_due_lease` را ۸۱۹۲ بایت نشان داد.** خطای
اندازه‌گیری یا برچسب نبود؛ توزیع Fixture بود. آن Fixture
(`seed(200000, 0.02, 0.02, 0.05)`) هر ردیف دارای `claim_expires_at` را هم‌زمان
دارای `next_attempt_at` می‌کرد، پس **صفر** ردیف در جریان `lease` می‌افتاد —
شمارش واقعی: ۱۹۰٬۰۰۰ `fresh` / **۰** `lease` / ۲٬۰۰۰ `retry` / ۸٬۰۰۰
`paired` — و Index خالی می‌ماند. جدول بالا روی توزیعی است که هر چهار جریان را
پر می‌کند.

**تقویت قفل.** هر جریان `FOR UPDATE SKIP LOCKED` را **پیش از** `LIMIT` خودش
می‌گیرد، پس هرکدام حداکثر `limit` ردیف قفل می‌کند: در بدترین حالت
`۴ × limit` قفل در برابر `limit` ردیف Claimشده. اندازه‌گیری‌شده با هر چهار
جریان پر: **۴۰۰ قفل / ۱۰۰ Claim**. نقشهٔ اجرا `Limit → LockRows → Index Scan`
است، پس قفل دقیقاً روی همان ردیف‌هایی می‌افتد که برگردانده می‌شوند، و این
قفل‌ها فقط تا پایان همین یک جمله زنده‌اند.

بدیلش گرسنگی است. با قفل‌کردن **پس از** ادغام، دو مدعی همان پنجرهٔ
پیش‌محدودشده را می‌سازند و دومی همه را قفل‌شده می‌بیند.
**اندازه‌گیری‌شده پیش از اصلاح:** ۳۰۰ ردیف واجد شرایط، صد ردیف قدیمی‌تر قفل،
`claimPending(100)` → **۰ ردیف**. شش آزمون Contention در
`outbox-durable-claim.int-spec.ts` این را می‌بندند.
