...
- **دامنه این ADR:** فقط Phase A — بررسی، بازتولید و تصمیم پیشنهادی.
  **هیچ کدی پیاده نشده است.** پذیرش و پیاده‌سازی، تصمیم جداگانهٔ صاحب محصول است.
...
**تصمیم پیشنهادی:** توالی هر جریان که **درون تراکنش دامنه و زیر قفل ردیفِ همان
...
**آنچه اسناد نمی‌گویند** و با فرض پر نمی‌شود: آیا برای جریان‌های
`fleet`/`maintenance` (کلید `assetId`) ترتیب **اکید** لازم است یا تشخیص شکاف
کافی است. این یک تصمیم محصول است و به‌عنوان **Q-36** ثبت شد.
...
تخصیص اولیهٔ پیشنهادی — با شواهد، نه سلیقه:
...
| `fleet`, `maintenance` (کلید `assetId`)                       | **باز** | **Q-36** — و R4 نشان داد قفل دامنه‌ای برای این مرز وجود ندارد |
...
STRICT این خودِ خواسته است. برای کلید داغ، هزینهٔ توان عملیاتی واقعی است و در
Q-36 و طرح اجرا ثبت شده.
...
| ۹   | Saga/ماشین وضعیت تحمل‌کنندهٔ بی‌ترتیبی | ⚠️              | ⚠️            | ⚠️           | ⚠️             | ⚠️          | برای برخی جریان‌ها درست است؛ تصمیم محصول ⇒ **Q-36**                                                 |
...
- **D-027 باز می‌ماند.** این ADR `Proposed` است. تا وقتی Phase B پیاده و
  تأیید نشده، هیچ تضمین تازه‌ای برقرار نیست.
- **Q-36** بی‌پاسخ است: کلاس جریان‌های `fleet`/`maintenance`.
