...
- `subjectType`/`subjectId` — _چیزی_ که اعلان دربارهٔ آن است (`InsurancePolicy`/`POL_x`)، نه رویداد.
...
> **ساخته نشده، و با NTF-004 می‌آید.** NTF-003 ترجیحات را بدون ساعات سکوت تحویل داد: `IN_APP` کسی را قطع نمی‌کند و در صندوق
> می‌ماند تا باز شود، پس تعویقش فقط یک ردیفِ موجود را پنهان و شمارندهٔ نخوانده را غلط می‌کند. این پاراگراف برای نخستین کانالی
> نوشته شده که می‌تواند کسی را بیدار کند. شرح انحراف در [برنامهٔ پیاده‌سازی § ۴](ADR-054-implementation-plan.md).

**اعلانِ الزامی.** `classification = MANDATORY` لایه‌های ۲ تا ۴ را برای دست‌کم یک کانال دور می‌زند. **این یک تصمیم محصول است، نه
مهندسی** — Q-38، که **در 2026-09-20 با همین پیش‌فرض بسته شد** و در NTF-003 پیاده است. پیش‌فرضِ حالا تصمیم‌شده: `MANDATORY` همیشه یک اعلان `IN_APP` تولید می‌کند (که نمی‌شود از آن انصراف داد، فقط
...
**قاعدهٔ اختصاصی: شکست Render یک رویداد Poison نیست.** متغیر غایب قالب یک `PERMANENT_FAILURE` روی _تحویل_ است، ثبت‌شده و دیده‌شده،
...
| **Q-38** | آیا کاربر می‌تواند از یک اعلان الزامی انصراف دهد |    **بسته ✅**     | **بسته ✅** — 2026-09-20، با NTF-003 |
| **Q-41** | آیا دلیل رد به شخص رد‌شده نشان داده می‌شود       |         نه         |                  نه                  |

**تنها Q-37 چیزی را مسدود می‌کند، و فقط نیمهٔ ایمیل COM-008 را.**

> **Q-38 دیگر باز نیست.** پاسخ ثبت‌شده در [`docs/24` Q-38](../24-open-questions.md) همان پیش‌فرض موقتِ § ۵ پایین است:
> `MANDATORY` همیشه `IN_APP` می‌سازد و هر کانال دیگری زیر کنترل ترجیح می‌ماند. NTF-003 آن را به‌صورت `mandatoryChannels`
> **به‌ازای هر قاعده** پیاده کرد، پس پاسخ بدون Migration عوض می‌شود. **هیچ قاعده‌ای امروز `MANDATORY` نیست**، پس این سیاست در
> سرویس مستقر هنوز هیچ اثری ندارد و `422` از آن بیرون نمی‌آید — به همین دلیل در قرارداد OpenAPI مستند نشده.
