...
> **پذیرش (2026-09-17 — صاحب محصول).** مرز مالکیت § ۱، رد Redis Redlock § ۴،
> مدل مستأجر سه‌نقشی § ۵، و تفکیک `Shipment`/`Fulfillment` § ۲ (با قاعدهٔ
> نمایش Q-44) اکنون **طراحی مصوب**اند — صاحب محصول این تصمیم‌ها را به همین
> شرکت واگذار و تأیید کرد. **این پذیرش کد نمی‌سازد و `COM-006` را Accepted
> نمی‌کند.** `services/inventory-service` همچنان وجود ندارد؛ `COM-006`
> همچنان `READY` و بدون امتیاز Story می‌ماند تا برنامهٔ اجرا طی شود و شواهد
> پذیرش § ۱۲ آن سند تأیید شوند. تغییر `planning/backlog.json` — وابستگی
...
**قاعدهٔ نمایش (Q-44، پاسخ صاحب محصول 2026-09-17).** اینکه UI خریدار وقتی هر
دو رکورد وجود دارند چه نشان دهد، یک قاعدهٔ محصولی نمایشی بود، نه یک تصمیم
معماری — و حدس زده نشد. صاحب محصول در `docs/24-open-questions.md` § Q-44
بستش: `Fulfillment` همیشه زیر برچسب **«اعلام تأمین‌کننده»** می‌آید و **هرگز
به‌تنهایی اثبات تحویل فیزیکی قلمداد نمی‌شود**؛ `Shipment` (وقتی وجود دارد)
زیر برچسب مستقل **«رهگیری محموله فیزیکی»**. وقتی هر دو موجودند و ناهم‌خوان‌اند،
UI یک **نشانگر ناهم‌خوانی خنثی** نشان می‌دهد — نه انتخاب خودکار یکی، نه
علامت‌گذاری خودکار سفارش به‌عنوان تحویل‌شده؛ گذار واقعی همچنان فقط از مسیر
`ConfirmReceipt` (اقدام صریح خریدار، ADR-038) می‌آید. این قاعده **پیاده
نشده**: ستون `Fulfillment.inventoryShipmentId`، Consumer پرکنندهٔ آن و
صفحهٔ سفارش ترکیبی هیچ‌کدام امروز وجود ندارند، و ساختشان نزد
`marketplace-service` می‌ماند — **خارج از دامنهٔ `COM-006`**.
...
هیچ‌کدام از این سه یکپارچگی امروز وجود ندارد. این ADR هرکدام را با یک Port
...
**نام‌گذاری این پورت‌ها به‌تنهایی `COM-006` را کامل نمی‌کند.** پذیرش
`COM-006` نیازمند شواهد واقعی § ۳ (همروندی رزرو) و § ۵/§ ۶ (رهگیری واقعی)
است — و «قابل‌مشاهده برای کاربر مجاز» یعنی یک انسان مجاز در یک نمای
واقعاً Render‌شده آن را می‌بیند، **نه اینکه یک تست API با توکن واقعی
پاسخ بگیرد**. برنامهٔ اجرا § ۱۲ این را با دو لایهٔ جدا (فنی و محصولی)
مشخص می‌کند؛ لایهٔ محصولی نیازمند یک Frontend است که امروز در این
Repository وجود ندارد.
...
این یافته `backlog.json` را **تغییر نمی‌دهد** — حذف وابستگی رسمی
`COM-006 → COM-005`، یا هر تغییر امتیاز Story، نیازمند یک **تغییر
برنامه‌ریزی جداگانه و قابل ردیابی** (تیکت/Commit مستقل با تأیید صاحب محصول)
است، نه یک اثر جانبی این ADR. برنامهٔ اجرا این تفکیک را به‌عنوان **فاز A**
(بدون‌وابستگی) و **فاز B** (منتظر یکپارچگی) دنبال می‌کند.
...
- `stock_item.available_quantity` به‌عنوان ستون Generated، Migration را
  ...
  دستی.
- قاعدهٔ نمایش ترکیبی `Fulfillment`+`Shipment` با Q-44 تصمیم‌گیری شد
  («اعلام تأمین‌کننده» در برابر «رهگیری محموله فیزیکی»، نشانگر ناهم‌خوانی
  خنثی) اما **پیاده نشده** — ستون اتصال، Consumer و صفحهٔ نمایش نزد
  `marketplace-service` می‌مانند، خارج از `COM-006`.
...
- **`docs/24-open-questions.md`.** `Q-44` برای قاعدهٔ نمایش
  `Fulfillment`/`Shipment` ثبت و در همین دور با پاسخ صاحب محصول (2026-09-17)
  بسته شد — UI‌اش همچنان پیاده نشده.
- **`docs/21-adr-list.md`.** ردیف `056` و خلاصهٔ تصمیم افزوده شد؛ وضعیت
  `Accepted` به‌روز شد.
- **`docs/adr/ADR-056-implementation-plan.md`.** گام ۱ (Scaffold) با الگوی
  واقعی اجرای محلی سرویس‌ها اصلاح شد؛ گام ۱۱ از ادعای «COM-006 کامل شد»
  تصحیح شد و یک گام جدید (§ ۱۲، شواهد پذیرش) افزوده شد.
...
- **AGENTS.md § ۹** — ابهام نمایش `Fulfillment`/`Shipment` حدس زده نشد؛
  Q-44 ثبت و با پاسخ صریح صاحب محصول (2026-09-17) بسته شد، نه با فرض ساکت.
...
- `docs/24-open-questions.md` Q-44 (این ADR، بسته)، Q-25، Q-33
