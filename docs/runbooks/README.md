...
| [audit-ingestion-lag](audit-ingestion-lag.md)       | `RastaAuditConsumerLag` — Lag مثبت پنج دقیقهٔ پیوسته در `audit-service.domain-projector` یا `audit-service.trail`؛ عمق نگه‌داشتهٔ `rasta.audit.v1.dlq` فقط Recording Rule است، بی هشدار | 🟠 هشدار   | ✅ آماده  |
...
«هشدار محرک» شرط مستند است، نه قاعدهٔ نوشته‌شده. رفتار این هشت قاعده با
...
- **Kafka از سمت Broker.** `kafka-exporter` (`danielqsj/kafka-exporter:v1.9.0`، Profile `observability`/`all`، بی Port میزبان)
  ...
  عمق Topicهای DLQ دیگر قاعده ندارند، و هشداری روی `up{job="kafka-exporter"}` نیست.
- **Label.** هر هشدار شمارنده با `sum by` فقط Labelهای کراندار خود متریک را نگه می‌دارد (هشدار Lag فقط `consumergroup, topic` و
  Recording Rule فقط `topic`)؛ هیچ Label یا Annotation شناسهٔ
