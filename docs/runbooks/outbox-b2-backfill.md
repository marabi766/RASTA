...
### رویداد پایانی هر سرویس، و آخرین خط

هر سرویس دقیقاً به یکی از سه حالت تمام می‌شود، و آخرین خط همیشه `summary` است:

| حالت     | رویداد پایانی | شمارندهٔ `summary` | معنا                                                                               |
| -------- | ------------- | ------------------ | ---------------------------------------------------------------------------------- |
| تمام‌شده | ندارد         | `ok`               | Plan، یا Apply ای که همگرا شد                                                      |
| ناتمام   | `incomplete`  | `incomplete`       | بودجهٔ `--max-batches` تمام شد یا ردیف بدون توالی مانده                            |
| امتناع   | `refused`     | `refused`          | `DATABASE_URL_<SERVICE>` تنظیم نشده، نقض پیش‌شرط B1، Guard ترتیب، یا شکست `VACUUM` |
...
**امتناع، مال همان سرویس است و نه اجرا.** هر سرویس مرز خطای خودش را دارد، از
جمله مرحلهٔ پیدا کردن اتصال. اگر `DATABASE_URL_<SERVICE>` سرویس نخست تنظیم
نشده باشد، همان سرویس یک `refused` می‌گیرد (که **نام متغیر** را می‌گوید، نه
مقدارش) و سرویس‌های بعدیِ انتخاب‌شده **همچنان اجرا می‌شوند**؛ `summary` هم مثل
همیشه چاپ می‌شود:
...
**کد خروج ۰ فقط وقتی است که هر سرویسِ انتخاب‌شده کاری را که خواسته شده تمام
کرده باشد**: Dry-Run همیشه به‌تعریف تمام است، و Apply وقتی تمام است که
`truncated: false` **و** `remaining: 0`. هر `incomplete` یا هر `refused` کد
خروج را ۱ می‌کند — پس `--apply --max-batches N` که کار باقی بگذارد **عمداً
غیرصفر** برمی‌گردد و اسکریپت اپراتور نمی‌تواند Backfill نیمه‌تمام را تمام‌شده
بخواند. همهٔ سرویس‌های انتخاب‌شده در هر حال امتحان می‌شوند؛ ایستادن یکی، بعدی را
رد نمی‌کند.

| اجرا                                                  | کد خروج |
| ----------------------------------------------------- | ------- |
| `--service document`                                  | ۰       |
| `--service document --apply` (همگرا)                  | ۰       |
| `--service document --apply --max-batches 1` (ناتمام) | **۱**   |
| هر امتناع                                             | **۱**   |
