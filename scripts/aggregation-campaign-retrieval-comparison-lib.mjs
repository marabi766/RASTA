 ...
 * lists that avoid command-line length limits. Their grammar, bounds and path
 * collision policy live in `aggregation-campaign-report-manifest-lib.mjs`,
 * shared with the accounting and image-cohort commands, and are re-exported
 * here under their earlier names. A manifest is operator-supplied transport
 * only: it proves nothing about authenticity or provenance, and its line order
 * means nothing because slots still come from report content.
 *
 * Pure: bytes and strings in, values out; nothing here touches a file
 * system. Diagnostics are fixed
...
import { REPORT_PATH_LIMITS } from './aggregation-campaign-report-manifest-lib.mjs';
import { MAX_CAMPAIGN_SLOT, MIN_CAMPAIGN_SLOT } from './aggregation-evidence-lib.mjs';

/** The shared manifest contract, under the names this comparator introduced. */
export {
  MAX_REPORT_MANIFEST_BYTES as MAX_RETRIEVAL_MANIFEST_BYTES,
  REPORT_MANIFEST_PROBLEM as MANIFEST_PROBLEM,
  findPathCollision,
  parseReportManifest as parseRetrievalManifest,
  pathApiFor,
  reportPathKey as retrievalPathKey,
...
export const RETRIEVAL_COMPARISON_LIMITS = Object.freeze({
  ...
  maxPathBytes: REPORT_PATH_LIMITS.maxPathBytes,
});
