...
import { SERVICE_NAME, CONSTRUCTION_TOPIC, type ConstructionEnv } from '../config/env';
import { validateConstructionPayload, type ConstructionEventName } from './events';
 ...
 * A-08): a cancelled project and the `PROJECT_STATUS_CHANGED` announcing it
 * commit together or not at all.
 *
 * ## The outbox row is written unscoped, and that is the standing exception
 *
 * The tenant guard scopes the domain models. `OutboxMessage` is exempt: it
...
export class EventPublisher {
  constructor(@Inject(ENV) private readonly env: ConstructionEnv) {}

  async enqueue<N extends ConstructionEventName>(
    ...
    const payload = validateConstructionPayload(input.eventName, input.payload);
    ...
    const streamSeq = await allocateStreamSeqSql(tx, CONSTRUCTION_TOPIC, partition.key);
 ...
 *
 * `PRJ` is the platform prefix for a project (`@rasta/contracts`
 * `ID_PREFIXES.project`); `PND` is this service's for a need, which the
 * platform list does not name. Organization-agnostic (AGENTS.md A-05): a ULID
 * and a type prefix, nothing that encodes a province, an organization type or
 * a tenant.
 */
