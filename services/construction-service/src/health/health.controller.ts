 ...
 * project is still created or cancelled, the row is still written and the event
 * is still durably queued — the relay drains it when Kafka returns (ADR-021).
 ...
 * been unreachable for an hour means audit-service is an hour behind.
 */
...
export class HealthController {
  ...
  async ready(@Res({ passthrough: true }) response: Response) {
    ...
    return {
      ...
      degraded,
      // Stated rather than left to be inferred: nothing in this service
      // approves, starts or completes a project on its own (ADR-063, Q-73).
