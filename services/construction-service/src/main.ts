...
import { loadConstructionEnv, corsOrigins, SERVICE_NAME } from './config/env';

const env = loadConstructionEnv();
...
async function bootstrap(): Promise<void> {
  ...
  // The largest legitimate payload is a project with a five-thousand-position
  // operating area (the DTO's own cap) and five thousand characters of scope;
  // anything much larger is malformed or hostile.
  app.useBodyParser('json', { limit: '512kb' });

  if (allowsDeveloperTooling(env)) {
    // The same document that is committed at
    // docs/api/construction-service.openapi.json (src/openapi/document.ts).
