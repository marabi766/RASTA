...
import { loadDocumentEnv, corsOrigins, SERVICE_NAME } from './config/env';

const env = loadDocumentEnv();
...
async function bootstrap(): Promise<void> {
  ...
  if (allowsDeveloperTooling(env)) {
    ...
    const document = SwaggerModule.createDocument(
      app,
      new DocumentBuilder()
        .setTitle('Rasta — Document Service')
        .setDescription(
          'Document metadata, object-level access control and short-lived signed URLs. ' +
            'The file itself lives in S3-compatible object storage and never passes ' +
            'through this service: clients upload directly to a signed URL and register ' +
            'the result (ADR-014). There is deliberately no upload endpoint here.',
        )
    ...
    SwaggerModule.setup(
      ...
      enrichOpenApiDocument(document, {
        // From the running configuration, so the published contract states the
        // durations this deployment actually honours rather than numbers
        // written beside them.
        signedUrlTtlSeconds: env.DOCUMENT_SIGNED_URL_TTL_SECONDS,
        uploadIntentTtlSeconds: env.DOCUMENT_UPLOAD_INTENT_TTL_SECONDS,
      }),
    );
  }
