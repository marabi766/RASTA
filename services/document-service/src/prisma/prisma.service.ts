 ...
 *
 * `Order` is scoped to the **buyer** (ADR-037 § 8), so a supplier reading its
 * own orders crosses this guard once, with a written reason, and lands in
 * `access.ts` where the check is explicit.
 *
 * `OutboxMessage` and `ProcessedEvent` are deliberately absent: the outbox is
 * platform plumbing written by a relay that has no request context, and it
 * carries its own tenant column for filtering. Same exception every other
