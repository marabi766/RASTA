...
import { FleetInternalController } from './internal.controller';

/**
 * The lock on the fact economic-service rewards usage from (ADR-061 § 4). The
...
describe('FleetInternalController', () => {
  it('lives under /internal, which the gateway routes nowhere, and admits only economic-service', () => {
    expect(Reflect.getMetadata(PATH_METADATA, FleetInternalController)).toBe(
      'internal/usage-records',
    );
    const handler = FleetInternalController.prototype.get;
    ...
    expect(Reflect.getMetadata(REQUIRED_ROLES_KEY, FleetInternalController)).toBeUndefined();
