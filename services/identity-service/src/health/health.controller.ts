...
export class HealthController {
  ...
  async ready(@Res({ passthrough: true }) response: Response) {
    ...
    // fails. Kafka being down delays event publication but the outbox retains
    // the events, so it must not take the service out of rotation (ADR-021).
  ...
  async startup(@Res({ passthrough: true }) response: Response) {
    const database = await this.prisma.isHealthy();
    const keycloak = await this.keycloak.isHealthy();

    // Keycloak matters at startup because a service that cannot provision
    // accounts is not finished starting - but not at readiness, where an
    // identity-provider blip should not take reads offline too.
    const started = database && keycloak;
    response.status(started ? HttpStatus.OK : HttpStatus.SERVICE_UNAVAILABLE);

    return { status: started ? 'ok' : 'starting', checks: { database, keycloak } };
  }
