...
export class HealthController {
  ...
  async ready(@Res({ passthrough: true }) response: Response) {
    ...
    // fails. Kafka being down delays event publication and stops usage
    // readings arriving, but the outbox retains what has not gone out and the
    // service still answers — and due state is derived, so a schedule still
    // reports correctly against the last reading that did arrive. Degraded,
    // not unready (ADR-021).
