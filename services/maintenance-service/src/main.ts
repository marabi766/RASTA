...
import { loadMaintenanceEnv, corsOrigins, SERVICE_NAME } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  if (allowsDeveloperTooling(env)) {
    ...
    const document = SwaggerModule.createDocument(
      app,
      new DocumentBuilder()
        .setTitle('Rasta — Maintenance Service')
        .setDescription(
          'What a machine needs, what was done to it, and what it cost. Service schedules ' +
            'evaluated on time or on usage, breakdown reports, repair orders, parts, labour ' +
            'and cost — up to the owner approval that settlement waits behind. The asset ' +
            'belongs to asset-service and usage to fleet-service; everything here references ' +
            'them by id and never reads their databases.',
        )
