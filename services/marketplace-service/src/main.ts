...
import { loadMarketplaceEnv, corsOrigins, SERVICE_NAME } from './config/env';
...
async function bootstrap(): Promise<void> {
  ...
  if (allowsDeveloperTooling(env)) {
    ...
    const document = SwaggerModule.createDocument(
      app,
      new DocumentBuilder()
        .setTitle('Rasta — Marketplace Service')
        .setDescription(
          'Who operates which machine, when, and for how long. Drivers, time-bounded ' +
            'assignments, usage readings and availability. The asset itself belongs to ' +
            'asset-service; everything here references it by id and never reads its database.',
        )
