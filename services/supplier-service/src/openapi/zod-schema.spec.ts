...
describe('the constructs the supplier DTOs are built from', () => {
...
describe('the real supplier DTOs', () => {
  it('publishes the register request as a closed object with every field', () => {
    const schema = toJsonSchema(registerSupplierSchema);
    const properties = schema.properties as Record<string, unknown>;

    expect(Object.keys(properties).sort()).toEqual(['capabilities', 'displayName']);
    // The property the whole authorization model depends on: there is no field
    // through which a client could name the organization, and the document
    // says so rather than leaving an integrator to discover it from a 400.
    expect(schema.additionalProperties).toBe(false);
    expect(properties).not.toHaveProperty('organizationId');
  });

  it('publishes the capability enum, so a client cannot guess a member', () => {
    const properties = toJsonSchema(registerSupplierSchema).properties as Record<
      string,
      { items?: { enum?: string[] } }
    >;

    expect(properties.capabilities.items?.enum).toEqual([
      'GOODS_SUPPLY',
      'WORKSHOP_SERVICE',
      'CONTRACTING',
    ]);
  ...
  it('publishes the rejection reason as required, with its minimum length', () => {
    // A decision with no reason is not an audit record, and a client should be
    // able to see the minimum before it sends "x".
    const schema = toJsonSchema(rejectQualificationSchema);

    expect(schema.required).toEqual(['reason']);
    expect((schema.properties as Record<string, { minLength: number }>).reason.minLength).toBe(8);
  });

  it('publishes evidence as identifiers and labels, and nothing fetchable', () => {
    const properties = toJsonSchema(submitQualificationSchema).properties as Record<
      string,
      { items?: { properties?: Record<string, unknown> } }
    >;
    const evidence = properties.evidence.items?.properties ?? {};

    expect(Object.keys(evidence).sort()).toEqual(['documentId', 'label']);
